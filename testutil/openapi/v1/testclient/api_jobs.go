...
func (r ApiJobsGetRequest) JobListRequest(jobListRequest JobListRequest) ApiJobsGetRequest {
	r.jobListRequest = &jobListRequest
	return r
}
...
func (a *JobsApiService) JobsGetExecute(r ApiJobsGetRequest) ([]JobListStub, *_nethttp.Response, error) {
	...
	localVarQueryParams := _neturl.Values{}
	localVarFormParams := _neturl.Values{}
	if r.jobListRequest == nil {
		return localVarReturnValue, nil, reportError("jobListRequest is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.jobListRequest
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["X-Nomad-Token"]; ok {
