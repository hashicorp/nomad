...
Once you have checked the [new version's upgrade details][upgrade-specific], the
upgrade process is as simple as updating the binary on each host and restarting
the Nomad service.
...
1. On another server issue a `nomad server members` and check the status, if
   the server is now in a left state you are safe to continue.

1. If the server is not in a left state, issue a `nomad server force-leave <server id>`
...
version details][upgrade-specific] page as some version
...
[`nomad node drain`]: /docs/commands/node/drain
