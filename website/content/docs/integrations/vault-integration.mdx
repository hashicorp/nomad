...
- `allowed_entity_aliases` - Specifies the entity aliases that can be used by
  this token. These aliases can be set in the (Nomad server config)[/docs/configuration/vault#nomad-server]
  to be used across all tasks, or on a task-by-task basis in (Nomad job config)[/docs/job-specification/vault#entity_alias].
