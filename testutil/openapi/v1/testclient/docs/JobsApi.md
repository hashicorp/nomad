...
[**JobsJobNameGet**](JobsApi.md#JobsJobNameGet) | **Get** /jobs/{jobName} | 
...
> Job JobsJobNameGet(ctx, jobName).Region(region).Namespace(namespace).Index(index).Wait(wait).Stale(stale).Prefix(prefix).XNomadToken(xNomadToken).PerPage(perPage).NextToken(nextToken).Execute()
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    api_client := openapiclient.NewAPIClient(configuration)
    resp, r, err := api_client.JobsApi.JobsJobNameGet(context.Background(), jobName).Region(region).Namespace(namespace).Index(index).Wait(wait).Stale(stale).Prefix(prefix).XNomadToken(xNomadToken).PerPage(perPage).NextToken(nextToken).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `JobsApi.JobsJobNameGet``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `JobsJobNameGet`: Job
    fmt.Fprintf(os.Stdout, "Response from `JobsApi.JobsJobNameGet`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiJobsJobNameGetRequest struct via the builder pattern
