...
Request frames represent process `stdin` stream as well as TTY resize events.
When ACLs are enabled, the first request frame must contain the authentication token.

The following are valid format
