...
export default class VersionedAllocationStatusBarComponent extends Component {
  ...
  get data() {
    ...
    // console.log('okay so', allocationsAtLatestVersion, Object.values(this.args.job.versionedSummary));
    return [
      {
        label: 'Queued',
        value: Object.values(this.args.job.versionedSummary).mapBy('Queued').map((status) => (status || {})[this.latestVersion.toString()]).reduce((m,n) => (m||0)+n) || 0,
        className: 'queued',
        // legendLink: this.generateLegendLink(this.job, 'queued'),
      },
      {
        label: 'Starting',
        value: Object.values(this.args.job.versionedSummary).mapBy('Starting').map((status) => (status || {})[this.latestVersion.toString()]).reduce((m,n) => (m||0)+n) || 0,
        className: 'starting',
        // legendLink: this.generateLegendLink(this.job, 'queued'),
      },
      {
        label: 'Running',
        value: Object.values(this.args.job.versionedSummary).mapBy('Running').map((status) => (status || {})[this.latestVersion.toString()]).reduce((m,n) => (m||0)+n) || 0,
        className: 'running',
        // legendLink: this.generateLegendLink(this.job, 'queued'),
      },
      {
        label: 'Complete',
        value: Object.values(this.args.job.versionedSummary).mapBy('Complete').map((status) => (status || {})[this.latestVersion.toString()]).reduce((m,n) => (m||0)+n) || 0,
        className: 'complete',
        // legendLink: this.generateLegendLink(this.job, 'queued'),
      },
      {
        label: 'Failed',
        value: Object.values(this.args.job.versionedSummary).mapBy('Failed').map((status) => (status || {})[this.latestVersion.toString()]).reduce((m,n) => (m||0)+n) || 0,
        className: 'failed',
        // legendLink: this.generateLegendLink(this.job, 'queued'),
      },
      {
        label: 'Lost',
        value: Object.values(this.args.job.versionedSummary).mapBy('Lost').map((status) => (status || {})[this.latestVersion.toString()]).reduce((m,n) => (m||0)+n) || 0,
        className: 'lost',
        // legendLink: this.generateLegendLink(this.job, 'queued'),
      },
