...
module('Acceptance | exec', function (hooks) {
  ...
  hooks.beforeEach(async function () {
    ...
    this.job = this.server.create('job', {
      groupsCount: 2,
      groupAllocCount: 5,
  ...
  test('/exec/:job should show the region, namespace, and job name', async function (assert) {
    ...
    let namespace = this.server.create('namespace');
    ...
    assert.deepEqual(Exec.header.region.text, 'region-2');
    assert.deepEqual(Exec.header.namespace.text, this.job.namespace);
    assert.deepEqual(Exec.header.job, this.job.name);
  ...
  test('/exec/:job should show the task groups collapsed by default and allow the tasks to be shown', async function (assert) {
    const firstTaskGroup = [...this.job.taskGroups.models].sort((a, b) => compare(get(a, 'name'), get(b, 'name')))[0];
    await Exec.visitJob({ job: this.job.id });

    assert.deepEqual(Exec.taskGroups.length, this.job.taskGroups.length);

    assert.deepEqual(Exec.taskGroups[0].name, firstTaskGroup.name);
  ...
  test('a task group with a pending allocation shows a loading spinner', async function (assert) {
    let taskGroup = [...this.job.taskGroups.models].sort((a, b) => compare(get(a, 'name'), get(b, 'name')))[0];
  ...
  test('an inactive task should not be shown', async function (assert) {
    let notRunningTaskGroup = [...this.job.taskGroups.models].sort((a, b) => compare(get(a, 'name'), get(b, 'name')))[0];
    this.server.db.allocations.update(
      { taskGroup: notRunningTaskGroup.name },
      { clientStatus: 'failed' },
    );

    let runningTaskGroup = [...this.job.taskGroups.models].sort((a, b) => compare(get(a, 'name'), get(b, 'name')))[1];
    ...
    assert.deepEqual(Exec.taskGroups[0].tasks.length, 1);
  });
  ...
  test('a task that becomes active should appear', async function (assert) {
    ...
    [...runningTaskGroup.tasks.models].sort((a, b) => compare(get(a, 'name'), get(b, 'name'))).forEach((task, index) => {
      let state = 'running';
      if (index > 0) {
        state = 'dead';
      }
    ...
    this.owner
      ...
      .peekAll('allocation')
      .forEach((allocation) => {
        const changingTaskState = allocation.states.find(item => get(item, 'name') === changingTaskStateName);
    ...
    assert.deepEqual(Exec.taskGroups[0].tasks.length, 2);
    assert.deepEqual(Exec.taskGroups[0].tasks[1].name, changingTaskStateName);
  });
  ...
  test('a dead job has an inert window', async function (assert) {
    ...
    let taskGroup = [...this.job.taskGroups.models].sort((a, b) => compare(get(a, 'name'), get(b, 'name')))[0];
    let task = [...taskGroup.tasks.models].sort((a, b) => compare(get(a, 'name'), get(b, 'name')))[0];
  ...
  test('visiting a path with a task group should open the group by default', async function (assert) {
    ...
    assert.deepEqual(Exec.taskGroups[0].tasks.length, taskGroup.tasks.length);
    assert.ok(Exec.taskGroups[0].chevron.isDown);
  ...
  test('a namespace can be specified', async function (assert) {
    this.server.create('namespace'); // default
    let namespace = this.server.create('namespace', {
    ...
    let taskGroup = [...job.taskGroups.models].sort((a, b) => compare(get(a, 'name'), get(b, 'name')))[0];
    let task = [...taskGroup.tasks.models].sort((a, b) => compare(get(a, 'name'), get(b, 'name')))[0];
    let allocations = this.server.db.allocations.where({
      jobId: job.id,
      taskGroup: taskGroup.name,
  ...
  test('running the command opens the socket for reading/writing and detects it closing', async function (assert) {
    ...
    let mockSockets = Service.extend({
      getTaskStateSocket(taskState, command) {
        assert.deepEqual(taskState.name, task.name);
        assert.deepEqual(taskState.allocation.id, allocation.id);
  ...
  test('the opening message includes the token if it exists', async function (assert) {
    const { secretId } = this.server.create('token');
  ...
  test('the command can be customised', async function (assert) {
    ...
    let taskGroup = [...this.job.taskGroups.models].sort((a, b) => compare(get(a, 'name'), get(b, 'name')))[0];
    let task = [...taskGroup.tasks.models].sort((a, b) => compare(get(a, 'name'), get(b, 'name')))[0];
    let allocation = this.server.db.allocations.find(item => get(item, {
      jobId: this.job.id,
      taskGroup: taskGroup.name,
    }));
