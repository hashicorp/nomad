...
module('Acceptance | variables', function (hooks) {
  ...
  hooks.beforeEach(async function () {
    faker.seed(1);
    this.server.createList('variable', 3);
  });
  ...
  test('it allows access for management level tokens', async function (assert) {
    ...
    await Variables.visit();
    assert.deepEqual(currentURL(), '/variables');
  ...
  test('it correctly traverses to and deletes a variable', async function (assert) {
    ...
    const variablesToken = this.server.db.tokens.find(VARIABLE_TOKEN_ID);
    window.localStorage.nomadTokenSecret = variablesToken.secretId;
    this.server.db.variables.update({ namespace: 'default' });
    const policy = this.server.db.policies.find('Variable-Maker');
  ...
  test('variables prefixed with nomad/jobs/ correctly link to entities', async function (assert) {
    ...
    await visit(`/jobs/${this.server.db.jobs[1].id}`);
    assert
      .dom('[data-test-task-stat="variables"]')
  ...
  test('it does not allow you to save if you lack Items', async function (assert) {
    ...
    window.localStorage.nomadTokenSecret = this.server.db.tokens[0].secretId;
    await Variables.visitNew();
    assert.deepEqual(currentURL(), '/variables/new');
    await typeIn('[data-test-path-input]', 'foo/bar');
  ...
  module('create flow', function () {
    test('allows a user with correct permissions to create a variable', async function (assert) {
      ...
      assert.deepEqual(currentRouteName(), 'variables.new');
  ...
  module('edit flow', function () {
    test('allows a user with correct permissions to edit a variable', async function (assert) {
      ...
      assert.deepEqual(currentRouteName(), 'variables.variable.index');
      assert
  ...
  module('read flow', function () {
    ...
    test('prevents users from reading a variable without proper permissions', async function (assert) {
      allScenarios.variableTestCluster(this.server);
      const variablesToken = this.server.db.tokens.find(
  ...
  module('namespace filtering', function () {
    ...
    test('does not show namespace filtering if the user only has access to one namespace', async function (assert) {
      ...
      const variablesToken = this.server.db.tokens.find(VARIABLE_TOKEN_ID);
      window.localStorage.nomadTokenSecret = variablesToken.secretId;
      const twoTokens = this.server.db.namespaces.slice(0, 2);
      this.server.db.namespaces.remove(twoTokens);
      await Variables.visit();

      assert.deepEqual(
        this.server.db.namespaces.length,
        1,
  ...
  module('Job Variables Page', function () {
    test('If the user has no variable read access, no subnav exists', async function (assert) {
      allScenarios.variableTestCluster(this.server);
      const variablesToken = this.server.db.tokens.find('n0-v4r5-4cc355');
      window.localStorage.nomadTokenSecret = variablesToken.secretId;
      await visit(
        `/jobs/${this.server.db.jobs[0].id}@${this.server.db.jobs[0].namespace}`,
      );
      ...
      await visit(
        `/jobs/${this.server.db.jobs[0].id}@${this.server.db.jobs[0].namespace}/variables`,
      );
      assert.deepEqual(currentURL(), '/jobs');
    ...
    test('If the user has variable read access, but no variables, the subnav exists but contains only a message', async function (assert) {
      ...
      window.localStorage.nomadTokenSecret = variablesToken.secretId;
      await visit(
        `/jobs/${this.server.db.jobs[1].id}@${this.server.db.jobs[1].namespace}`,
      );
      assert.dom('[data-test-tab="variables"]').exists();
      await click('[data-test-tab="variables"] a');
      assert.deepEqual(
        currentURL(),
        `/jobs/${this.server.db.jobs[1].id}@${this.server.db.jobs[1].namespace}/variables`,
      );
    ...
    test('Multiple task variables are included, and make a maximum of 1 API request', async function (assert) {
      ...
      let requests = this.server.pretender.handledRequests.filter(
        (request) =>
          request.url === '/v1/vars?path=nomad%2Fjobs' ||
          request.url === `/v1/vars?prefix=nomad%2Fjobs%2F${job.name}`,
      );
      assert.deepEqual(requests.length, 2);
    ...
    test('The intro text shows examples of variables at groups and tasks', async function (assert) {
      ...
      this.server.db.taskGroups.forEach((group) => {
        this.server.create('variable', {
      ...
      assert
        ...
        .hasAttribute(
          'href',
          `/ui/variables/var/nomad/jobs/${job.id}/${this.server.db.taskGroups[0].name}@${job.namespace}/edit`,
        );
      ...
      assert
        ...
        .hasAttribute(
          'href',
          `/ui/variables/new?path=nomad%2Fjobs%2F${job.id}%2F${this.server.db.taskGroups[0].name}%2F${this.server.db.tasks[0].name}`,
        );
      assert
        .dom('[data-test-variables-intro-tasks] code:nth-of-type(2) a')
        .hasAttribute(
          'href',
          `/ui/variables/new?path=nomad%2Fjobs%2F${job.id}%2F${this.server.db.taskGroups[0].name}%2F${this.server.db.tasks[1].name}`,
        );
    });
  });
});
