...
func TestReconciler_Disconnected_Client(t *testing.T) {
	...
	testCases := []testCase{
		...
		// TODO (derek): What if it's flapping and replacements are pending?
		{
		...
		{
			name:                    "ignore-original-completed",
			allocCount:              2,
		...
		},
		{
			...
			expected: &resultExpectation{
				...
				desiredTGUpdates: map[string]*structs.DesiredUpdates{
					"web": {
						Stop: 2,
						// Ignore the 5 running and 2 failed replacements - test ensures 2 stop are the old jobs
						Ignore: 7,
					},
				},
			},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.maxDisconnect != nil {
				reconciler.now = time.Now().Add(*tc.maxDisconnect * 5)
			}
