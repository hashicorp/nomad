...
- `client_introduction` <code>([ClientIntroduction](#client_introduction-parameters))</code> -
  Configuration for how the Nomad server will handle client introduction
  requests.
...
- `enforcement` `(string: "warn")` - Specifics how the server will handle new
  client registration requests. The valid options are `none`, `warn`, and
  `strict`.

  When set to `none`, the server will not enforce any client registration
  behaviour. When set to `warn`, the server will log a warning message and emit
  a telemetry metric when a client attempts to register without an introduction
  token, but will allow the registration to proceed. When set to `strict`, the
  server will reject any client registration attempts that do not include a
  valid introduction token and also log a warning message and emit a telemetry
  metric.
...
- `default_identity_ttl` `(string: "5m")` - The default TTL assigned to
  ...
  This is specified using a label suffix like "30s" or "1h" and must be less
  than `max_identity_ttl`.

- `max_identity_ttl` `(string: "30m")` - The maximum TTL that can be requested
  by a caller when generating client introduction tokens. This is specified
  using a label suffix like "30s" or "1h" and must be greater than
  `default_identity_ttl`.

The [Monitoring Nomad][monitoring_nomad_client_introduction] has more details on
the client introduction metric that is emitted.
...
[monitoring_nomad_client_introduction]:/nomad/docs/monitor#client_introduction
