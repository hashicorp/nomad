...
```shell-session
$ curl \
    https://localhost:4646/v1/job/my-job/actions
```
...
This endpoint executes a predefined action from a job specification in the
...
- `allocID` `(string: <required>)` - Specifies the UUID of the target
  allocation. Specified as a query parameter.
- `task` `(string: <required>)` - Specifies the task within the group where the
  action is defined. Specified as a query parameter.
...
To execute an action, a WebSocket upgrade request is sent. The request headers
...
$ curl \
  ...
  http://127.0.0.1:4646/v1/job/actions-demo/action?action=weather&allocID=8614ed07-425f-889f-170d-dd2716e0e01f&task=task&tty=true -o /dev/tty
```
