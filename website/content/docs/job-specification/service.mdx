...
- `weights` <code>([Weights][weights]: nil)</code> - Specifies how a service
  instance is weighted in a DNS SRV request based on the service's health status.
  Only available where `provider = "consul"`.
