...
- `task/`: Each task has a **task working directory** with the same name as
  ...
  - `task/local`: This directory is the location provided to the task as the
    `NOMAD_TASK_DIR`. Note this is not the same as the "task working
    directory". This directory is private to the task.

  - `task/secrets`: This directory is the location provided to the task as
  ...
  - `task/tmp/`: A temporary directory used as scratch space by task drivers.
...
$ nomad alloc fs c0 c0b2245f alloc/
Mode        Size     Modified Time         Name
...
$ nomad alloc fs c0 c0b2245f task1/
Mode         Size     Modified Time         Name
