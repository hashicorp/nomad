...
- `config_file` - The `lxc` configuration file to load (commonly defaults to `/etc/lxc/default.conf`). See [`lxc.conf`][lxc-conf] for more details.
...
[lxc-conf]: https://linuxcontainers.org/lxc/manpages/man5/lxc.conf.5.html
