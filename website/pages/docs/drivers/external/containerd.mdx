...
- `host_dns` - (Optional) `true` (default) or `false` By default, a container
  ...
  similar to [`docker behavior`][]. However, if you don't want to use
...
A [`service stanza`](https://www.nomadproject.io/docs/job-specification/service)
can be added to your job spec, to enable service discovery.
...
[`docker behavior`]: https://docs.docker.com/config/containers/container-networking/#dns-services
