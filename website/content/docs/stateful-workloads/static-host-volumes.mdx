...
Nomad cluster. This guide walks you through deploying a MySQL workload using
...
sandbox environment. This guide will assume a cluster with one server node and three
client nodes.

<Note>

This guide is for demo purposes and only assumes a single server
node. Consult the [reference architecture][reference-arch] for
...
- The `read_only` argument is supplied on all of the volume-related stanzas in
  ...
  read-only volume mount. Review the [`host_volume`], [`volume`], and
...
This guide is for demo purposes and does not follow best
...
Once you re-connect to MySQL, the information you
