...
this command to manually generate the report and send it to HashiCorp. If no
snapshots were persisted in the last 24 hrs, Nomad takes a new snapshot.
...
- `-message` `(string; "")` - Provide context about the conditions under which
  ...
  utilization bundle but will be included in the Nomad server logs.
