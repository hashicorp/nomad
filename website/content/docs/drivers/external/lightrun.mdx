...
the job spec. To find these configurations, log into the [Lightrun Management Portal][lightrun_portal] 
and look inside the **Download the Agent** section:

- `lightrun_server` - The Lightrun server you are working against, including your company name.
- `lightrun_secret` - Lightrun secret is used for authorization against the Lightrun server.
