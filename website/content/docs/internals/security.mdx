...
description: Learn about Nomad's security requirements and recommendations.
---

## Overview

Nomad is a flexible workload orchestrator, which deploys and manages any application, 
containerized or non-containerized, in a unified workflow.

Using defense in depth through multiple security controls is crucial for Nomad 
cluster security. Some security features for multi-tenant deployments are offered 
exclusively in the enterprise version. 

The general mechanisms for a secure Nomad deployment revolve around:
...
- **[mTLS](https://learn.hashicorp.com/tutorials/nomad/security-enable-tls)** -
  ...
  components within the cluster. This should be enabled for both the HTTP and 
  the RPC endpoints on all agents within the cluster. A reverse proxy, or client 
  certificates installed in the browser, or obtained from Vault for programmatic
  access may be required to allow API access.

- **[ACLs](https://learn.hashicorp.com/collections/nomad/access-control)** - 
  Enables authorization for authenticated mTLS connections by granting capabilities 
  to ACL tokens.

- **[Namespaces](https://learn.hashicorp.com/tutorials/nomad/namespaces)** - 
  Access to read, and write to a namespace can be controlled to allow for granular 
  access to job information managed within a multi-tenant cluster.

- **[Sentinel Policies](https://learn.hashicorp.com/tutorials/nomad/sentinel)** <EnterpriseAlert inline /> - 
  Sentinel policies allow for granular control over components such as task drivers within a cluster.

### Personas

It helps to consider the following types of personas when managing the security 
requirements of a Nomad deployment. The granularity may change depending on your 
organization's requirements.

-> **Note:** There's no traditional concept of a user within Nomad.

- **System Administrator** - This is someone who has access to the underlying
  infrastructure to a Nomad cluster. Often they have access to SSH or RDP
...
- **Nomad Administrator** - This is someone (probably the same **System
  ...
  for servers and clients; they might have a Nomad management ACL token. They also
...
- **[mTLS](https://learn.hashicorp.com/tutorials/nomad/security-enable-tls)** - 
  ...
  * Observing or altering communication between nodes is thwarted due to the
  ...
  * Agent role misconfiguration is prevented using the X.509
    ...
    Nomad uses a SAN to encode the node's role and region. These SANs
    can be validated to ensure that members of the cluster
    are performing their expected functions. 
    These SANs are resemble domain names and are generally
    expressed as `role.region.nomad` (for example: 
    `client.us-east.nomad`).

  * Using the previously mentioned role name prevents maliciously masquerading
    as a server or client node. It also allows other services to be signed easily by
    the same CA. This also avoids any potential pitfalls with certificates using
    the IP, or Hostname of nodes within a cluster.

- **[ACLs](https://learn.hashicorp.com/collections/nomad/access-control)** - 
  The access control list (ACL) system provides a capability-based control
...
- **[Resource Quotas](https://learn.hashicorp.com/tutorials/nomad/quotas)** <EnterpriseAlert inline /> - 
  Can limit a namespace's access to the underlying compute resources in the 
  cluster by setting upper-limits for operators. Access to these resource quotas 
  can be managed via ACLs to ensure read-only access for operators so they can't 
  just change their quotas.

- **[Sentinel Policies](https://learn.hashicorp.com/tutorials/nomad/sentinel)** <EnterpriseAlert inline /> - 
  [Sentinel](https://www.hashicorp.com/sentinel/) enables [policy-as-code](https://docs.hashicorp.com/sentinel/concepts/policy-as-code/) to enforce further restrictions on operators. This is used to augment the
...
- **Rotate credentials** - Using short-lived credentials and rotating them
  frequently is highly recommended to reduce damage of accidentally leaked
  credentials.

  - Create and manage credentials with [Vault](/docs/integrations/vault-integration). These
    ...
    This helps prevent leaked credentials, which can be committed to version control or stored
  ...
  - Rotate credentials used by the Nomad agent; e.g. [integrate with Vault's
    ...
    dynamically generate and automatically renew unique X.509 certificates for each
...
- **[Disable Unused Drivers](/docs/configuration/client#driver-denylist)** -
  ...
  unintended privilege escalation. If a task driver is not needed, you should 
...
- **Linux Security Modules** - Use of security modules that can be directly
  ...
  both the Nomad hosts and containers for an extra layer of security. Seccomp 
  profiles are able to be passed directly to containers using the
...
- **[Service Mesh](https://www.hashicorp.com/resources/service-mesh-microservices-networking)** -
  ...
  **[Consul](https://www.consul.io/)** can be extremely useful for restricting 
  and efficiently load balancing network connectivity within a cluster.

- **[TLS Settings](/docs/configuration/tls)** -
  TLS settings, such as the available [cipher suites](/docs/configuration/tls#tls_cipher_suites), 
  should be tuned to fit the needs of your environment.

- **[HTTP Headers](/docs/configuration#http_api_response_headers)** -
  Additional security [headers](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers) can 
  be [configured](/docs/configuration#http_api_response_headers) for HTTP API responses.
...
- **Nomad agent-to-agent communication** - Transport encryption for
  ...
  encryption. These mechanisms include symmetric (shared gossip encryption keys) and
  asymmetric keys (TLS).

- **Tampering of data in transit** - Any tampering should be detectable via mTLS,
  which will cause Nomad to avoid processing the request.

- **Access to data without authentication or authorization** - Requests to the
  server should be authenticated using mTLS and authorized with Nomad ACLs.
...
- **Memory access to a running Nomad server agent** - Direct access to the
  ...
  being compromised including access to certificates, tokens, and other secrets.
...
- **Job Operator** - Someone with a valid mTLS certificate and a valid ACL token may still be a
