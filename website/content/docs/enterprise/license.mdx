...
Nomad Enterprise can be used without a valid license for 6 hours. When Nomad
...
shows log excerpts of the warnings. Since this was a temporary license, when
...
of an autoscaling group. If an operator accidentially has an old, expired
license set as the disk or environment license, the server will emit a warning
log, but not exit in case a valid license exists in raft. If a valid license
