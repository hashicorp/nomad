...
- `topic` `(topic:filter_key: "*:*")` - Specifies a topic to subscribe to and
  ...
  `Deployment` events for a job redis. To only subscribe to `Node` events a
  topic parameter of `?topic=Node` without a separator value would be used.
  `?topic=Node:*` is also valid.
