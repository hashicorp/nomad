...
parameters section below). The `logs` stanza allows for finer-grained control
...
- `enabled` `(bool: true)` - Controls if Nomad should write and rotate logfiles.
  You can disable this setting if you're running e.g. some thirdparty log manager.
