...
func (s *Server) replicateNodePools(stopCh chan struct{}) {
	...
	for {
		...
		if !s.peerPartsCache.ServersMeetMinimumVersion(s.Region(), minNodePoolsVersion, true) {
			s.logger.Trace(
...
func (s *Server) schedulePeriodic(stopCh chan struct{}) {
	...
	for {
		...
		case <-oneTimeTokenGC.C:
			if !s.peerPartsCache.ServersMeetMinimumVersion(
				s.Region(),
...
func (s *Server) reconcileMember(serfMem serf.Member) error {
	...
	valid, parts := peer.IsNomadServer(serfMem)
...
func (s *Server) addRaftPeer(m serf.Member, parts *peer.Parts) error {
	// Check for possibility of multiple bootstrap nodes
	serfMems := s.serf.Members()
	if parts.Bootstrap {
		for _, serfMem := range serfMems {
			valid, p := peer.IsNomadServer(serfMem)
...
// or is reaped
func (s *Server) removeRaftPeer(m serf.Member, parts *peer.Parts) error {
	addr := (&net.TCPAddr{IP: m.Addr, Port: parts.Port}).String()
...
func (s *Server) replicateACLRoles(stopCh chan struct{}) {
	...
	for {
		...
		default:
			...
			if !s.peerPartsCache.ServersMeetMinimumVersion(s.Region(), minACLRoleVersion, true) {
				s.logger.Trace(
...
func (s *Server) replicateACLAuthMethods(stopCh chan struct{}) {
	...
	for {
		...
		default:
			...
			if !s.peerPartsCache.ServersMeetMinimumVersion(s.Region(), minACLAuthMethodVersion, true) {
				s.logger.Trace(
...
func (s *Server) replicateACLBindingRules(stopCh chan struct{}) {
	...
	for {
		...
		default:
			...
			if !s.peerPartsCache.ServersMeetMinimumVersion(s.Region(), minACLBindingRuleVersion, true) {
				s.logger.Trace(
...
func (s *Server) getOrCreateAutopilotConfig() *structs.AutopilotConfig {
	...
	if !s.peerPartsCache.ServersMeetMinimumVersion(peer.AllRegions, minAutopilotVersion, false) {
...
func (s *Server) getOrCreateSchedulerConfig() *structs.SchedulerConfiguration {
	...
	if !s.peerPartsCache.ServersMeetMinimumVersion(s.Region(), minSchedulerConfigVersion, false) {
...
func (s *Server) initializeKeyring(stopCh <-chan struct{}) {
	...
	for {
		...
		if s.peerPartsCache.ServersMeetMinimumVersion(s.Region(), minVersionKeyring, true) {
			break
		}
	}
	...
	isClusterUpgraded := s.peerPartsCache.ServersMeetMinimumVersion(
...
func (s *Server) generateClusterMetadata() (structs.ClusterMetadata, error) {
	if !s.peerPartsCache.ServersMeetMinimumVersion(peer.AllRegions, minClusterIDVersion, false) {
