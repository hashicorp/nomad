...
`consul-cni` plugin on each client node to redirect inbound and outbound traffic for services to the Envoy proxy. Install the reference CNI plugins before you install the Consul CNI plugin. The following commands assume that you have installed the reference CNI plugins.
