...
// PartsCache is a threadsafe cache of known Nomad server peers parsed from Serf
...
func (p *PartsCache) PeerSet(parts *Parts) {
	p.peersLock.Lock()
...
func (p *PartsCache) PeerDelete(event serf.MemberEvent) {
	p.peersLock.Lock()
	defer p.peersLock.Unlock()
...
func (p *PartsCache) ServersMeetMinimumVersion(
	region string,
