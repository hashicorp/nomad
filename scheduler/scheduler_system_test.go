...
func TestSystemSched_JobRegister_StickyAllocs(t *testing.T) {
	...
	require.NoError(t, h.State.UpdateAllocsFromClient(structs.MsgTypeTestSetup, h.NextIndex(), time.Now().UnixNano(), []*structs.Allocation{alloc}))
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.previousTerminal {
				...
				require.NoError(t, h.State.UpsertAllocs(structs.MsgTypeTestSetup, h.NextIndex(), time.Now().UnixNano(), []*structs.Allocation{prev}))
			}
...
func TestSystemSched_CSITopology(t *testing.T) {
	...
	must.NoError(t, h.State.UpsertCSIVolume(
		h.NextIndex(), time.Now().UnixNano(), []*structs.CSIVolume{vol0}))
