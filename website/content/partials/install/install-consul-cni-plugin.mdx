When you use the [`transparent_proxy` block](/nomad/docs/job-specification/transparent_proxy) for Consul service mesh, you must also install the
[`consul-cni` plugin](https://releases.hashicorp.com/consul-cni) on each client node for Consul to properly redirect inbound and outbound traffic for services to the Envoy proxy. For more information, refer to [Enable the Consul CNI plugin](/consul/docs/k8s/connect/transparent-proxy/enable-transparent-proxy#enable-the-consul-cni-plugin) in the Consul documentation.
