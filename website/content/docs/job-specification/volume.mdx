...
- `sticky` `(bool: false)` - Specifies that this volume is supposed to "stick"
  ...
  available. "Sticky" volumes are used for [stateful deployments].
  The `sticky` field can only be used for dynamic host volumes. For CSI volumes,
  `per_alloc` field provides similar functionality.
...
- `per_alloc` `(bool: false)` - Specifies that the `source` of the volume
  ...
  that use canaries. `per_alloc` is mutually exclusive with `sticky`
  property—the former can only be used with CSI volumes, and the latter only
