...
- [Host volumes plugin
  ...
  of how to write your own plugin to dynamically configure persistent storage on your Nomad client nodes
...
- The [`nomad acl auth-method create`](/nomad/docs/commands/acl/auth-method/create) CLI command
- The [`nomad acl auth-method update`](/nomad/docs/commands/acl/auth-method/update) CLI command
...
- [OIDC troubleshooting guide][oidc-trouble] to review common issues and
  tips for setting up OIDC
...
Showing UI URL hints is enabled by default. Modify the [`show_cli_hints`
parameter](/nomad/docs/configuration/ui#show_cli_hints) in your agent's `ui`
block configuration to turn off this feature.
