...
validation and also store the check index in disk so it can be used later to
...
parsing. Adding `|| true` at the end prevents undesired failures since
`nomad job plan` returns a non-zero exit code if a change is detected.
