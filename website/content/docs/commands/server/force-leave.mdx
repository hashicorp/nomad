---
layout: docs
page_title: 'nomad server force-leave reference'
description: >
  The `nomad server force-leave` command force a server into the left state so that you can eject server nodes that have failed.
---

# `nomad server force-leave` reference
