...
- by storing explicit repository credentials or by specifying Docker
  `credHelpers` in a file and setting the auth [config](/nomad/docs/deploy/task-drivers/docker#config)
  value on the client in the plugin options.

- by specifying an auth [helper](/nomad/docs/deploy/task-drivers/docker#helper) on the client in the
  plugin options.
