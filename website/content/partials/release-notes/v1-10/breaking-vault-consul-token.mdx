...
| ------ | ------------ | ------------ |
| [`vault.allow_unauthenticated`](/nomad/docs/configuration/vault#allow_unauthenticated) | Agent | Tasks should use a workload identity. Do not use a Vault token. |
| [`vault.task_token_ttl`](/nomad/docs/configuration/vault#task_token_ttl) | Agent | With workload identity, tasks receive their TTL configuration from the Vault role. |
| [`vault.token`](/nomad/docs/configuration/vault#token) | Agent | Nomad agents use the workload identity when making requests to authenticated endpoints. |
| [`vault.policies`](/nomad/docs/job-specification/vault#policies)  | Job specification  |  Configure and use a Vault role.  |
