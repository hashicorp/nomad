...
1. Create a native application. Refer to the Auth0 [Create Applications
   ...
   1. Note the **Domain**, **Client ID**, and **Client Secret** values. You use
   these to configure an auth method in Nomad.
   1. Set the callback URLs that Auth0 uses to redirect the user after the
...
1. Create a role called `engineering`. Refer to the the Auth0 [Create Roles guide](https://auth0.com/docs/manage-users/access-control/configure-core-rbac/roles/create-roles) for instructions.
1. Create a user and assign the `engineering` role to that user. Refer to these
   Auth0 guides for instructions:
...
1. Create a [login
   ...
   1. Create a [post-login
      ...
      **Actions** and then **Triggers**.
      1. Select **post-login**.
      1. In the **Add Action** pane, **Custom** tab, select **Nomad OIDC** and
...
the Nomad access to your Auth0 App. Then log in to Auth0 using
