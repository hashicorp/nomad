...
- `:node_id` `(string: <optional>)` - Specifies the node to target. This is
  required when the endpoint is being accessed via a server is specified as part
  of the path (`?node_id=...`). Note, this must be the _full_ node ID, not the
...
- `NodeID` or `:node_id` `(string: <optional>)` - Specifies the node to target.
  ...
  (`NodeID: "..."`). Note, this must be the _full_ node ID, not the short
  8-character one.
