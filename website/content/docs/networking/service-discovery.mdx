...
To access services, other allocations can query the catalog using [`template`]
blocks with the [`service`][ct_service_fn] function to query the Consul catalog
or the [`nomadService`][ct_nomad_service_fn] function when using Nomad native
service discovery. The `template` can then be used as a configuration file or
have its content loaded as environment variables to configure connection
...
set of tags for the canary allocations using the [`canary_tags`] configuration.
