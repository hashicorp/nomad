...
Specify locations (region, zone, rack, etc.) where the provisioned CSI volume
must be accessible from or from where an existing volume is accessible from. The
`topology_request` block is not supported for dynamic host volumes.
