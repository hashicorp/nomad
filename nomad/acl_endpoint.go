	...
	opts := blockingOptions{
		...
		run: func(ws memdb.WatchSet, store *state.StateStore) error {
			...
			if prefix := args.QueryOptions.Prefix; prefix != "" {
				iter = store.ACLPolicyByNamePrefix(ws, prefix)
			} else {
...
func (a *ACL) ListTokens(args *structs.ACLTokenListRequest, reply *structs.ACLTokenListResponse) error {
	...
	opts := blockingOptions{
		...
		run: func(ws memdb.WatchSet, store *state.StateStore) error {
			...
			if prefix := args.QueryOptions.Prefix; prefix != "" {
				iter = store.ACLTokenByAccessorIDPrefix(ws, prefix, sort)
				tokenizer = paginator.IDTokenizer[*structs.ACLToken](args.NextToken)
			} else if args.GlobalOnly {
				iter = store.ACLTokensByGlobal(ws, true, sort)
				tokenizer = paginator.IDTokenizer[*structs.ACLToken](args.NextToken)
			} else {
...
func (a *ACL) ListRoles(
	...
	return a.srv.blockingRPC(&blockingOptions{
		...
		run: func(ws memdb.WatchSet, store *state.StateStore) error {
			...
			default:
				iter = store.GetACLRoleByIDPrefix(ws, args.QueryOptions.Prefix)
			}
