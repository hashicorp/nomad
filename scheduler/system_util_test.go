...
func TestDiffSystemAllocs(t *testing.T) {
	...
	*oldJob = *job
	...
	if len(diff.update) > 0 {
		require.Equal(t, allocs[0], diff.update[0].Alloc) // first alloc should be updated
	}

	if len(diff.ignore) > 0 {
		require.Equal(t, allocs[1], diff.ignore[0].Alloc) // We should ignore the second alloc
	}
