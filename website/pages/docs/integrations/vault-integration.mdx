...
```shell-sessionvault operator init -key-shares=1 -key-threshold=1
...
```shell-sessionvault operator unseal
...
```shell-sessionvault login <your initial root token>
...
```shell-sessionvault policy write nomad-server nomad-server-policy.hcl
...
```shell-sessionvault write /auth/token/roles/nomad-cluster @nomad-cluster-role.json
...
```shell-sessionsudo systemctl restart nomad
...
```shell-sessionnomad run db.nomad

```

Verify the job is running with the following command:

```shell-sessionnomad status postgres-nomad-demo
...
```shell-sessionvault secrets enable database
...
```shell-sessionvault write database/config/postgresql @connection.json
...
```shell-sessionvault write database/roles/accessdb db_name=postgresql \
...
```shell-sessionvault read database/creds/accessdb
...
```shell-sessionvault policy write access-tables access-tables-policy.hcl
