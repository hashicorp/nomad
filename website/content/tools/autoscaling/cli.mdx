...
- `-high-availability-lock-path`: Defines the path of the Nomad variable that
  will be used to sync the leader when running on high availability mode. This
...
- `-high-availability-lock-delay`: Defines the period that the lock will be
  blocked after the TTL expired without being renewed. It is helps prevent split
