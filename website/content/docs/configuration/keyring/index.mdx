...
described in [Key Management][]. Nomad refers to this as `"aead"`. Using the
`keyring` configuration block, Nomad servers can instead use an external key management system (KMS) or
...
`active=true`, and restart the servers. The server starts using the new
keyring wrapper when the current key is rotated either periodically or via the
[`nomad operator root keyring rotate`][keyring_rotate_cmd] command.
...
startup, Nomad tries each KMS provider in order until it finds a provider
