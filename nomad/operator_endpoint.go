...
func (op *Operator) AutopilotSetConfiguration(args *structs.AutopilotSetConfigRequest, reply *bool) error {
	...
	if !op.srv.peerPartsCache.ServersMeetMinimumVersion(op.srv.Region(), minAutopilotVersion, false) {
...
func (op *Operator) SchedulerSetConfiguration(args *structs.SchedulerSetConfigRequest, reply *structs.SchedulerSetConfigurationResponse) error {
	...
	if !op.srv.peerPartsCache.ServersMeetMinimumVersion(
		op.srv.Region(),
...
func (op *Operator) forwardStreamingRPCToServer(server *peer.Parts, method string, args interface{}, in io.ReadWriteCloser) error {
