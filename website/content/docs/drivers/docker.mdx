...
- `container_exists_attempts` - (Optional) A number of attempts to be made if
  during task creation Nomad encounters an existing container in non-running
  state for the same task. Defaults to `5`.
