...
Nomad ignores the `client.disk_free_mb` field. Nomad also no longer attempts to
calculate the free disk space, as this changes when clients with running
allocations restart. We recommend that you reserve at least the amount of disk
that is used by the host OS.
