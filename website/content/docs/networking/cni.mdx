...
This page show you how to install the CNI reference plugins on your Linux distribution and configure your Nomad clients to use the plugins.
...
install plugins on each client

configure nomad clients with plugin directory etc
...
Nomad reads the following extensions from the [`cni_config_dir`](/nomad/docs/configuration/client#cni_config_dir) — `/opt/cni/config` by default:
...
network [`mode`](/nomad/docs/job-specification/network#mode) attribute to the value `cni/<your_cni_config_name>`.
...
for the `bridge` network mode, using the [loopback](https://github.com/containernetworking/plugins#main-interface-creating), [bridge](https://www.cni.dev/plugins/current/main/bridge/), [firewall](https://www.cni.dev/plugins/current/meta/firewall/),
and [portmap](https://www.cni.dev/plugins/current/meta/portmap/) CNI plugins configured together to create Nomad's bridge
network.
...
values for [`bridge_network_name`](/nomad/docs/configuration/client#bridge_network_name), [`bridge_network_subnet`]( /nomad/docs/configuration/client#bridge_network_subnet), and an
