		...
		run: func(ws memdb.WatchSet, store *state.StateStore) error {
			...
			if err == structs.ErrPermissionDenied {
				...
				if prefix := args.QueryOptions.Prefix; prefix != "" {
					iter, err = store.AllocsByIDPrefix(ws, namespace, prefix, sort)
					tokenizer = paginator.IDTokenizer[*structs.Allocation](args.NextToken)
				} else if namespace != structs.AllNamespacesSentinel {
					iter, err = store.AllocsByNamespaceOrdered(ws, namespace, sort)
...
func (a *Alloc) GetServiceRegistrations(
	...
	return a.srv.blockingRPC(&blockingOptions{
		...
		run: func(ws memdb.WatchSet, stateStore *state.StateStore) error {
			...
			services, err := stateStore.GetServiceRegistrationsByAllocID(ws, args.AllocID)
			if err != nil {
				return err
			}

			reply.Services = services.Slice()
