...
would be filtered. As of 1.4.0, `stages` and `operations` are treated as `AND filters`. Logs will only be filtered if all filter conditions match.
...
behavior to reserve for a major Nomad release and _not_ be backported. Please
...
can use `nomad eval list -json` to get that list in JSON format. The `nomad eval status <eval ID>` command will format a specific evaluation in JSON format if
