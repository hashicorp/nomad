...
consul lock -monitor-retry=60 -shell .insecure_root_token /ops/shared/scripts/bootstrap_vault.sh || true
sleep 1
VAULT_TOKEN="$(consul kv get .insecure_root_token)"
# Nomad

sed -i "s/SERVER_COUNT/$SERVER_COUNT/g" $CONFIGDIR/nomad.hcl
sed -i "s/VAULT_TOKEN/$VAULT_TOKEN/g" $CONFIGDIR/nomad.hcl
sed -i "s/VAULT_ADDR/$VAULT_ADDR/g" $CONFIGDIR/nomad.hcl
