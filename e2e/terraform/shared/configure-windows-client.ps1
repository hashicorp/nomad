...
# Consul
cp "C:\opt\shared\consul\consul_client_$cloud.json" C:\opt\consul.d\config.json
sc.exe create "Consul" binPath= "C:\opt\consul.exe" agent -config-dir "C:\opt\consul.d" start= auto
sc.exe start "Consul"

# Vault
cp "C:\opt\shared\vault\vault.hcl" C:\opt\vault.d\vault.hcl
sc.exe create "Vault" binPath= "C:\opt\vault.exe" agent -config-dir "C:\opt\vault.d" start= auto
sc.exe start "Vault"

# Nomad
# TODO: we need some Windows builds!
md C:\opt\nomad
aws s3 cp "s3://nomad-team-test-binary/builds-oss/${nomad_sha}-windows.tar.gz" "nomad.tar.gz"
Expand-Archive .\nomad.tar.gz C:\opt\nomad.exe
