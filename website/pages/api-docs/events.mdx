...
description: |-
  The /events endpoints interact with the local Nomad agent to interact with
  members and servers.
---
...
This endpoint streams a servers backlog off events as we as new events as they occur.
...
the nature of this endpoint different topics require different policies.
...
- `index` `(int: 0)` - Specifies the index to start streaming events from. If
  the requeste index is no longer in the buffer the stream will start at the
  nearest index.

- `topic` `(topic:filter_key: "*:*")` - Specifies a topic to subscribe to and
  filter on. By default all topics are subscribed to. Multiple topics should be
  ...
  `:`. As an example `Deployment:redis` would subscribe to all `Deployment`
  events for a job redis. To only subscribe to `Node` events a topic parameter
  or `Node` without a separator value would be used.
  ...
  "Events": [
    {
      ...
      "Payload": {
        "Node": {
          "ID": "afb9c810-d701-875a-b2a6-f2631d7c2f60",
