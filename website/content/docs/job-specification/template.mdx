...
Nomad [variables] can be queried using the `nomadVar`, `nomadVarSafe`,
`nomadVarList`, `nomadVarListSafe`, and `nomadVarExists` functions.
...
task. The path filter can change the a namespace by adding a suffix separated
by the `@` character:
...
task. The path filter can change the a namespace by adding a suffix separated by
the `@` character:
...
The `nomadVarSafe` function works identically to `nomadVar`, but refuses to
render the template if the variable query returns blank/empty data.
