...
- `work_dir` - (Optional) The working directory for the task. Like
  command, this path must be within the task's [chroot](#chroot) or in a [host volume][] mounted with a
  [`volume_mount`][volume_mount] block.
