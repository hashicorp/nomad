...
Preemption is enabled by default in Nomad 0.9. Operators can use the [scheduler config](/api/operator#update-scheduler-configuration) API endpoint to disable preemption.
...
Operators can use the [allocation API](/api/allocations#read-allocation) or the `alloc status` command to get visibility into
