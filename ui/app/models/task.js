...
export default class Task extends Fragment {
  ...
  get pathLinkedVariable() {
    if (!this._job) {
      ...
      return this._job.variables?.find(item => get(item, 'path') === `nomad/jobs/${jobID}/${this.taskGroup.name}/${this.name}`);
    }
  }
  ...
  async getPathLinkedVariable() {
    ...
    return await this._job.variables?.find(item => get(item, 'path') === `nomad/jobs/${jobID}/${this.taskGroup.name}/${this.name}`);
  }
}
