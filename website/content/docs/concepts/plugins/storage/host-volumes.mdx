...
This page describes the plugin specification for [dynamic host volumes][dhv],
with examples, so you can write your own plugin to dynamically configure
persistent storage on your Nomad client nodes.
...
get_path() {
  ...
  if [ -z "$DHV_HOST_PATH" ]; then
    stderr "DHV_HOST_PATH must not be empty"
    exit 1
  fi
  ...
  echo "$DHV_HOST_PATH/$DHV_VOLUME_NAME"
}
...
A plugin is registered with Nomad if it:
...
* Must complete within 5 seconds, or Nomad will kill it
...
DHV_HOST_PATH=/suggested/path/to/place/volume
...
DHV_VOLUME_ID={volume ID assigned by Nomad}
...
* Returned "path" must be the location of the created volume.
  Nomad will mount this path into workloads that request the volume.
...
Nomad also runs `delete` when an initial volume `create` operation
fails, since it may have been only partially completed.
...
DHV_HOST_PATH=/suggested/path/to/place/volumes
DHV_NODE_ID={Nomad node ID}
DHV_NODE_POOL={Nomad node pool}
DHV_NAMESPACE={volume namespace}
DHV_VOLUME_NAME={name from the volume specification}
...
* Must complete within 60 seconds, or Nomad will kill it
* Must be idempotent - repeat runs produce the same result
* Will be run after a partial (failed) create operation TODO TODO TODO !!! how about create=UPDATE ???
* Must be safe to run concurrently, per volume name per node
