...
description: |-
  The "disconnect" block describes the behaviuor of both the Nomad server and 
...
The `disconnect` block describes the systm's behaviour in case of a network 
partition.
...
- `replace` `(bool: false)` - Specifies if the disconnected allocation should 
  ...
  node it is running on becomes disconnected or goes down, this allocations
  wont be rescheduled and will show up as `unknown` until the node comes back 
  up or it are manually stoped:
...
- `stop_after` `(string: "")` - Specifies a duration after which a disconnected 
  Nomad client will stop its allocations. Keep in mind that if stop_after is 
  shorter than ttl, and `replace` = false, can lead to a state where no allocations
  are running, hence it is not admitted and will cause a validation error.

  The Nomad client process must be running for this to occur. This setting
  cannot be used with [`replace_after`].
...
- `reconcile` `(string: “best_score”)` - Specifies which allocation to keep once
  ...
  It has four possible values which are described below:
        * `keep_original`: Always keep the original allocation, bare in mind 
...
[`replace_after`] : /nomad/docs/job-specification/disconnect#replace_after
...
`lost_after` specifies a duration during which a Nomad client will
...
the one with the best node score. If the `lost_after` duration
...
rather than "down" until the last `lost_after` duration has expired.
