...
export const JobDeploymentTaskGroups = <template>
      ...
      >
        ...
        <t.body as |row|>
          <tr data-test-deployment-task-group>
            ...
            <td data-test-deployment-task-group-auto-revert>{{if
                row.model.autoRevert
                "Yes"
                "No"
              }}</td>
            <td data-test-deployment-task-group-canaries>{{or
                row.model.placedCanaries
                0
              }}
              /
              {{row.model.desiredCanaries}}</td>
            <td
              data-test-deployment-task-group-allocs
            >{{row.model.placedAllocs}}
              /
              {{row.model.desiredTotal}}</td>
            <td
              data-test-deployment-task-group-healthy
            >{{row.model.healthyAllocs}}</td>
            <td
              data-test-deployment-task-group-unhealthy
            >{{row.model.unhealthyAllocs}}</td>
            <td data-test-deployment-task-group-progress-deadline>
              <span class="nowrap">{{formatTs
