...
- `command` `(string: "")` - Specifies the full path to a script or executable
  that is to be executed on template change. Path is relative to the driver, e.g.,
  if running with a container driver the path must be existing in the container.
  This option is required is the `change_mode` is `script`.
...
- `allowed_exit_codes` `(array<int>: [0])` - List of exit codes that are
  considered clean, i.e., if a script exits with one of the codes in that list and
  `fail_on_error` is true, Nomad will not kill the task. 
