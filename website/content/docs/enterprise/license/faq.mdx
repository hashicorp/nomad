...
expiration time is compared to the build date of the Nomad binary, which you can see with `nomad version`.

The practical result is that newer contract licenses will work in perpetuity for any v1.6+ version of Nomad
...
- [v1.6.0 Upgrade Guide](/nomad/docs/upgrade/upgrade-specific#nomad-1-6-0-enterprise)
...
then a Nomad server will not be able to start using a new v1.6 binary.
...
## Q: May I downgrade Nomad Enterprise from v1.6 to an earlier version?

Yes, if you upgrade a server to 1.6 but it fails to start due to license expiration,
