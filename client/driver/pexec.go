...
const (
	...
	pexecConfigOption = "driver.pexec.enable"
...
// The PexecDriver is a privileged version of the exec driver. It provides no
// resource isolation and just fork/execs. The Exec driver should be preferred
// and this should only be used when explicitly needed.
type PexecDriver struct {
	DriverContext
}

// pexecHandle is returned from Start/Open as a handle to the PID
type pexecHandle struct {
	proc   *os.Process
	waitCh chan error
	doneCh chan struct{}
}

// NewPrivilegedExecDriver is used to create a new privileged exec driver
func NewPrivilegedExecDriver(ctx *DriverContext) Driver {
	return &PexecDriver{*ctx}
}

func (d *PexecDriver) Fingerprint(cfg *config.Config, node *structs.Node) (bool, error) {
	// Check that the user has explicitly enabled this executor.
	enabled := strings.ToLower(cfg.ReadDefault(pexecConfigOption, "false"))
	if enabled == "1" || enabled == "true" {
		d.logger.Printf("[WARN] driver.Pexec: privileged exec is enabled. Only enable if needed")
		node.Attributes["driver.pexec"] = "1"
		return true, nil
	}
...
func (d *PexecDriver) Start(ctx *ExecContext, task *structs.Task) (DriverHandle, error) {
	// Get the command
	command, ok := task.Config["command"]
	if !ok || command == "" {
		return nil, fmt.Errorf("missing command for pexec driver")
	}
...
func (d *PexecDriver) Open(ctx *ExecContext, handleID string) (DriverHandle, error) {
	// Split the handle
...
func (h *pexecHandle) ID() string {
	// Return a handle to the PID
	return fmt.Sprintf("PID:%d", h.proc.Pid)
}

func (h *pexecHandle) WaitCh() chan error {
	return h.waitCh
}

func (h *pexecHandle) Update(task *structs.Task) error {
