...
- `max_parallel` `(int: 1)` - Specifies the number of allocations within a task
  ...
  ~> **Note:** System jobs only support 1 allocation per node. Updates when
  `canary` is set will make up to `max_parallel` destructive updates *or*
...
- `canary` `(int: 0)` - Specifies that changes to the job that would result in
  ...
  nodes to which Nomad will make destructive allocation updates. System jobs do
  not support more than one allocation per node, so effectively setting `canary`
  to a positive integer means this percentage of feasible nodes will get a new
  ...
  `canary` is set, nodes that register during a deployment will not receive
  placements until after the deployment is promoted.

  ~> **Note:** Updates when `canary` is set will make up to `max_parallel`
