...
inbound and outbound connections without being aware of service mesh at all.
...
service mesh to the counting service via localhost on port 9001. The proxy is managed
...
**Note**: Nomad's service mesh integration requires Consul in your `$PATH`
...
```shell-session
$ sudo nomad agent -dev-service mesh
```
