...
Nomad uses the Hashicorp Configuration Language version 2 -  
([HCL2](https://github.com/hashicorp/hcl/blob/hcl2/README.md)) designed to allow
concise descriptions of the required steps to get to a job file. HCL2 extends the
HCL language by adding expressions and input variables support to improve job spec
...
assignment syntax. The following [`mounts`](/nomad/docs/drivers/docker#mounts) syntax is not valid:
...
Or better yet, the new [`mount`](/nomad/docs/drivers/docker#mount) syntax, is more appropriate here:
