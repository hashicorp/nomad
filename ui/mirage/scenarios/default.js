...
function smallCluster(server) {
  ...
  server.schema.allocations.find(item => get(item, { taskGroup: actionsGroup.name })).update({
  ...
  const variableLinkedGroup = server.db.taskGroups.find(item => get(item, {
    jobId: variableLinkedJob.id,
  }));
  const variableLinkedTask = server.db.tasks.find(item => get(item, {
