...
- `credentials` `(string: <required>)`: The path to the credentials JSON file to
  use. May be also specified by the `GOOGLE_CREDENTIALS` or
...
that order, if the above GCP specific values are not provided.
...
`cloudkms.cryptoKeys.get` permission is used for retrieving metadata information
...
deleted and are used to decrypt older data. Any new or updated data will be
