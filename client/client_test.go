...
func TestClient_DefaultIneligible(t *testing.T) {
	...
	assert.Equal(t, structs.NodeSchedulingIneligible, c1.Node().SchedulingEligibility)
	...
	testutil.WaitForResult(func() (bool, error) {
		...
		assert.Equal(t, structs.NodeSchedulingIneligible, out.Node.SchedulingEligibility)
		return out.Node.ID == req.NodeID, nil
	}, func(err error) {
		t.Fatalf("err: %v", err)
	})
