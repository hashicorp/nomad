...
// RPCLayer implements the raft.StreamLayer interface,
...
// NewRPCLayer is used to initialize a new RaftLayer which can
// be used as a StreamLayer for Raft. If a tlsConfig is provided,
// then the connection will use TLS.
func NewRPCLayer(addr net.Addr, tlsWrap tlsutil.Wrapper) *RPCLayer {
	layer := &RPCLayer{
		addr:    addr,
...
func (l *RPCLayer) Handoff(ctx context.Context, c net.Conn) error {
	select {
	case l.connCh <- c:
		return nil
	case <-l.closeCh:
...
func (l *RPCLayer) Accept() (net.Conn, error) {
	select {
	case conn := <-l.connCh:
		return conn, nil
	case <-l.closeCh:
...
func (l *RPCLayer) getTLSWrapper() tlsutil.Wrapper {
	l.tlsWrapLock.RLock()
...
func (l *RPCLayer) ReloadTLS(tlsWrap tlsutil.Wrapper) {
	l.tlsWrapLock.Lock()
...
func (l *RPCLayer) Addr() net.Addr { return l.addr }

// Dial is used to create a new outgoing connection
func (l *RPCLayer) Dial(address raft.ServerAddress, timeout time.Duration) (net.Conn, error) {
