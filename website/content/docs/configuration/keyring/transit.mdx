...
the values set in the server's [`vault`][vault_config] block. You must set required fields either here or the `vault` block.

- `address` `(string: <required>)`: The full address to the Vault cluster.  Alternately specify via the `VAULT_ADDR` environment variable.

- `token` `(string: <required>)`: The Vault token to use.  Alternately specify via the `VAULT_TOKEN` environment variable.
...
The Vault authentication token needs the following permissions on the
transit key:
...
endpoints. Refer to [Vault docs][rotate_doc]. Old keys must not be disabled or deleted and
