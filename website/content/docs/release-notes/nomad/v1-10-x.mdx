...
these volumes and risk data loss, so we enhanced garbage collection to help you
prevent orphaned dynamic host volumes.
...
Adds `once` mode to the template block. This allows templates to render once
without watching dependencies. Refer to the following resources for more
information:
...
Templates: We removed support for these non-hermetic sprig functions:
