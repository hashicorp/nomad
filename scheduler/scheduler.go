...
type State interface {
	...
	NodesByNodePool(ws memdb.WatchSet, poolName string) (state.ResultIterator[*structs.Node], error)
	...
	AllocsByNode(ws memdb.WatchSet, node string) (state.ResultIterator[*structs.Allocation], error)
	...
	// CSIVolumeByID fetch CSI volumes, containing controller jobs
	CSIVolumesByNodeID(memdb.WatchSet, string, string) (state.ResultIterator[*structs.CSIVolume], error)
	...
	// given node
	HostVolumesByNodeID(memdb.WatchSet, string, state.SortOption) (state.ResultIterator[*structs.HostVolume], error)
