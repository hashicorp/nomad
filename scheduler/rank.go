	...
	for {
		...
		for _, task := range iter.taskGroup.Tasks {
			...
			if task.Resources.Cores > 0 {
				// set of reserveable CPUs for the node
				nodeCPUSet := cpuset.FromSlice(option.Node.NodeResources.Cpu.ReservableCpuCores)
				// set of all reserved CPUs on the node
				allocatedCPUSet := cpuset.NewCPUSet()
				for _, alloc := range proposed {
					allocatedCPUSet = allocatedCPUSet.Union(cpuset.FromSlice(alloc.ComparableResources().Flattened.Cpu.ReservedCores))
				}

				// set of CPUs not reserved on the node
