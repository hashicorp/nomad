...
- `guest_agent` `(bool: false)` - Enable the [QEMU Guest 
  ...
  file in the task's working directory for interacting with the agent, which
  must be running in the VM to work.  This feature is currently not supported
  on Windows.
