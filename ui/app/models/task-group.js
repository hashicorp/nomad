...
export default class TaskGroup extends Fragment {
  ...
  get pathLinkedVariable() {
    if (this.job.parent.get('id')) {
      return this.job.variables?.find(
        (item) =>
          get(item, 'path') ===
      ...
      return this.job.variables?.find(
        (item) =>
          get(item, 'path') === `nomad/jobs/${this.job.plainId}/${this.name}`
      );
    }
  }
  ...
  async getPathLinkedVariable() {
    ...
    if (this.job.parent.get('id')) {
      return await this.job.variables?.find(
        (item) =>
          get(item, 'path') ===
          `nomad/jobs/${this.job.parent.get('plainId')}/${this.name}`
      );
  ...
  get allocations() {
    return maybe(this.get('job.allocations')).filter(
      (item) => get(item, 'taskGroupName') === this.name
    );
  }
  ...
  get placementFailures() {
    ...
    return (
      placementFailures &&
      placementFailures.find((item) => get(item, 'name') === this.name)
    );
  }
  ...
  get summary() {
    return maybe(this.get('job.taskGroupSummaries')).find(
      (item) => get(item, 'name') === this.name
    );
  }

  @computed('job.scaleState.taskGroupScales.[]', 'name')
  get scaleState() {
    return maybe(this.get('job.scaleState.taskGroupScales')).find(
      (item) => get(item, 'name') === this.name
    );
  }
