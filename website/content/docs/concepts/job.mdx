...
new job. After you select the version to clone, Nomad opens a window where you
can edit, plan, and run the new version.

Because Nomad passes job version attributes via query params, you can copy the
URL on the **Jobs** run page to send someone a link to a readily-editable job
spec.
