	...
	testCases := []struct {
	...
	}{
		{
			...
			setup: func(m *MockEventlog) {
				m.ExpectInfo(uint32(EventLogMessage), "Information line")
				m.ExpectWarning(uint32(EventLogMessage), "Warning line")
				m.ExpectError(uint32(EventLogMessage), "Error line")
			},
		},
		{
			desc:  "higher level",
		...
		},
		{
			...
			setup: func(m *MockEventlog) {
				...
				m.ExpectWarning(uint32(EventLogMessage), "Warning line")
				m.ExpectError(uint32(EventLogMessage), "Error line")
				m.ExpectInfo(uint32(EventLogMessage), "Information log\nthat includes\nmultiple lines")
				m.ExpectWarning(uint32(EventLogMessage), "Warning log\nthat includes second line")
			},
			msgs: append(defaultmsgs, []string{
