...
When Nomad is configured with [`client_introduction.enforcement`] set to either
`warn` or `strict`, the server handling the client registration request will
increment the `nomad.client.introduction.enforcement` counter each time a new
...
an introduction token which is important when migrating to a tougher enforcement
level.
