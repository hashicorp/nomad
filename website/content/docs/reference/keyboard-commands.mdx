...
The Nomad Web UI supports several keyboard shortcuts in order to help users
navigate and operate Nomad. You can use keyboard commands to inspect jobs,
view logs, monitor evaluations, and more.

## Open the shortcuts panel

Type a question mark (`?`) with on any web UI page to display the
**Keyboard Shortcuts** panel. The available shortcuts and their corresponding
commands are shown in the table below. Type the first letter in the command,
then the second letter to open the page.
...
1. Click on the toggle next to the **Keyboard shortcuts enabled** option at the
bottom of the panel
...
1. Open the **Keyboard Shortcuts** panel.
1. Click on the appropriate key combination next to the shortcut you want to
re-bind.
...
number in the tooltip hint next to the item you want to navigate to. Note that
items with a single digit require a leading zero (0).