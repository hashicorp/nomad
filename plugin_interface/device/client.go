...
func (d *devicePluginClient) Fingerprint(ctx context.Context) (<-chan *FingerprintResponse, error) {
	...
	if err != nil {
		return nil, helper.HandleReqCtxGrpcErr(err, ctx, d.doneCtx)
	}
		...
		if err != nil {
			if err != io.EOF {
				out <- &FingerprintResponse{
					Error: helper.HandleReqCtxGrpcErr(err, reqCtx, d.doneCtx),
				}
			}
...
func (d *devicePluginClient) Reserve(deviceIDs []string) (*ContainerReservation, error) {
	...
	if err != nil {
		return nil, helper.HandleGrpcErr(err, d.doneCtx)
	}
