...
func (s *StateStore) CSIVolumesByNamespace(ws memdb.WatchSet, namespace, prefix string, sort SortOption) (memdb.ResultIterator, error) {
	txn := s.db.ReadTxn()
	...
	switch sort {
	case SortReverse:
		iter, err = txn.GetReverse("csi_volumes", "id_prefix", namespace, prefix)
	default:
