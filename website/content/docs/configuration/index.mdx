...
- `limits` `(`[`Limits`]`: nil)` - Available in Nomad 0.10.3 and later, this is
  a nested object that configures limits that are enforced by the agent.
...
[`limits`]: /docs/configuration/limits 'Nomad Agent limit Configuration'
