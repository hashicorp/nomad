...
func (s *Server) AllowClientOpInCallerPool(ctx *RPCContext, aclObj *acl.ACL, args structs.RequestWithIdentity) bool {
	if aclObj == nil {
		return false
	}

	pool := resolveCallerNodePool(s, ctx, args.GetIdentity())
	return aclObj.AllowClientOp(pool)
}
