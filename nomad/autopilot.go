...
func (s *Server) MinRaftProtocol() (int, error) {
	return minRaftProtocol(s.serf.Members(), peer.IsNomadServer)
}
...
func minRaftProtocol(members []serf.Member, serverFunc func(serf.Member) (bool, *peer.Parts)) (int, error) {
	minVersion := -1
...
func (s *Server) autopilotServerFromMetadata(srv *peer.Parts) (*autopilot.Server, error) {
