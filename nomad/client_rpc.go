...
func (s *Server) serverWithNodeConn(nodeID, region string) (*peer.Parts, error) {
	// We skip ourselves.
	...
	var rawTargets []*peer.Parts
	if region == s.Region() {
		rawTargets = make([]*peer.Parts, 0, len(s.localPeers))
	...
	targets := make([]*peer.Parts, 0, len(rawTargets))
	...
	// requested node.
	var mostRecentServer *peer.Parts
