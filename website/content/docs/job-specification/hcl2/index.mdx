...
Since HCL is a superset of JSON, `nomad job run example.json` would attempt to
parse a JSON job using the HCL parser. However when JSON formatted jobs are
desired, the preferred approach is to use the [API's JSON
format][json-jobs-api] and the [`-json` command line flag][run-json]:
...
The HCL parser's JSON format is unspecified, so the API format is preferred.
