...
- `ChangeScriptConfig` - Configures the script triggered on template change.
  This option is required if the `ChangeMode` is `script`.
  ...
  - `path` - Specifies the full path to a script that is to be executed on
    template change. Path is relative to the driver, e.g., if running with a
    container driver the path must be existing in the container. This option is
    required is the `change_mode` is `script`.
  ...
  - `fail_on_error` - If `true`, Nomad will kill the task if the script
    ...
    continue uninterrupted. Default value is `false`.
