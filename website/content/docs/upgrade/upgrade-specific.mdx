...
`sysbatch` jobs in each client they are running. This feature makes an API call
to an endpoint that requires `node:read` ACL permission. Tokens used to access
the Nomad UI will need to be updated to include this permission in order to
access a job details page.
