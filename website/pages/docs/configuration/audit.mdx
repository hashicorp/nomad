...
### `audit` Configuration Reloads

Nomad supports dynamically reloading the audit configuration. To reload an
agent's audit configuration, first update the `audit` block in the agent's
configuration file and then send the Nomad agent a `SIGHUP` signal.
