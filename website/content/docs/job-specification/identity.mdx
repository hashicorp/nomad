...
job "docs" {
  group "example" {
    task "api" {
      ...
      # ... rest of task specification ...
    }
  }
}
```

The empty block `identity {}` will expose both the environment variable and
file, but either may be individually disabled:
...
- `env` `(bool: true)` - If true the workload identity will be available in the
  task's `NOMAD_TOKEN` environment variable.
- `file` `(bool: true)` - If true the workload identity will be available in
  the task's filesystem via the path `secrets/nomad_token`.
