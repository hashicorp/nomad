...
func (d *driverPluginClient) handleFingerprint(reqCtx context.Context, ch chan *Fingerprint, stream proto.Driver_FingerprintClient) {
	...
	for {
		...
		if err != nil {
			if err != io.EOF {
				ch <- &Fingerprint{
					Err: helper.HandleReqCtxGrpcErr(err, reqCtx, d.doneCtx),
				}
			}
...
func (d *driverPluginClient) RecoverTask(h *TaskHandle) error {
	...
	return helper.HandleGrpcErr(err, d.doneCtx)
}
...
func (d *driverPluginClient) StartTask(c *TaskConfig) (*TaskHandle, *DriverNetwork, error) {
	...
	if err != nil {
		...
		return nil, nil, helper.HandleGrpcErr(err, d.doneCtx)
	}
...
func (d *driverPluginClient) handleWaitTask(ctx context.Context, id string, ch chan *ExitResult) {
	...
	if err != nil {
		result.Err = helper.HandleReqCtxGrpcErr(err, ctx, d.doneCtx)
	} else {
...
func (d *driverPluginClient) TaskStats(ctx context.Context, taskID string, interval time.Duration) (<-chan *TaskResourceUsage, error) {
	...
	ch := make(chan *TaskResourceUsage, 1)
	go d.handleStats(ctx, ch, stream)

	return ch, nil
}

func (d *driverPluginClient) handleStats(ctx context.Context, ch chan<- *TaskResourceUsage, stream proto.Driver_TaskStatsClient) {
	defer close(ch)
	for {
...
func (d *driverPluginClient) CreateNetwork(allocID string, _ *NetworkCreateRequest) (*NetworkIsolationSpec, bool, error) {
	...
	if err != nil {
		return nil, false, helper.HandleGrpcErr(err, d.doneCtx)
	}
