...
func getFuzzyMatchesImpl[T comparable](iter memdb.TableResultIterator[T], text string, limitQuery, limitResults int) (map[structs.Context][]structs.FuzzyMatch, map[structs.Context]bool) {
...
func getPrefixMatches(contexts []structs.Context, aclObj *acl.ACL, namespace, prefix string, ws memdb.WatchSet, store *state.StateStore) (map[structs.Context][]string, map[structs.Context]bool, error) {
	...
	for _, context := range contexts {
		...
		case structs.Allocs:
			iter, err := store.AllocsByIDPrefix(ws, namespace, prefix, state.SortDefault)
			if err != nil {
				return ids, truncs, err
			}
			ids[context], truncs[context] = prefixMatches(iter,
		...
		case structs.Nodes:
			iter, err := store.NodesByIDPrefix(ws, prefix)
			if err != nil {
				return ids, truncs, err
			}
			ids[context], truncs[context] = prefixMatches(iter,
		...
		case structs.NodePools:
			...
			if !aclObj.IsManagement() {
				iter = memdb.NewTableFilterIterator(iter, nodePoolCapFilter(aclObj))
			}
			ids[context], truncs[context] = prefixMatches(iter,
				func(pool *structs.NodePool) string { return pool.Name })

		case structs.Deployments:
			iter, err := store.DeploymentsByIDPrefix(ws, namespace, prefix, state.SortDefault)
			if err != nil {
				return ids, truncs, err
			}
			ids[context], truncs[context] = prefixMatches(iter,
		...
		case structs.ScalingPolicies:
			iter, err := store.ScalingPoliciesByIDPrefix(ws, namespace, prefix)
			if err != nil {
				return ids, truncs, err
			}
		...
		case structs.HostVolumes:
			iter, err := store.HostVolumesByIDPrefix(ws, namespace, prefix, state.SortDefault)
			if err != nil {
				return ids, truncs, err
			}
			ids[context], truncs[context] = prefixMatches(iter,
		...
		case structs.Namespaces:
			...
			iter = memdb.NewTableFilterIterator(iter, func(ns *structs.Namespace) bool {
		...
		default:
			iter, err := getEnterpriseResourceIter(context, aclObj, namespace, prefix, ws, store)
			if err != nil {
				return ids, truncs, err
			}
			ids[context], truncs[context] = prefixMatchesOld(iter,
...
func prefixMatches[T comparable](iter memdb.TableResultIterator[T], getID func(T) string) ([]string, bool) {
	ids := []string{}
	isTrunc := false
	i := 0
...
// prefixMatchesOld is a hack so that we don't have to update 100% of iterator
// queries in the state store yet
//
// TODO(tgross): finish the job
func prefixMatchesOld(iter memdb.ResultIterator, getID func(any) string) ([]string, bool) {
	ids := []string{}
	isTrunc := false
	i := 0
	for {
...
// wrong namespace).
func nsCapIterFilter[T comparable](iter memdb.TableResultIterator[T], aclObj *acl.ACL) memdb.TableResultIterator[T] {
	return memdb.NewTableFilterIterator(iter, nsCapFilter[T](aclObj))
}

// nsCapFilter produces a memdb.FilterFunc for removing objects not accessible
// by aclObj during a table scan.
func nsCapFilter[T comparable](aclObj *acl.ACL) memdb.TableFilterFunc[T] {
...
func nodePoolCapFilter(aclObj *acl.ACL) memdb.TableFilterFunc[*structs.NodePool] {
	...
	for _, context := range contexts {
		...
		case structs.Jobs:
			...
			if wildcard(namespace) {
				iter, _ = store.Jobs(ws, state.SortDefault)
				iter = nsCapIterFilter(iter, aclObj)
			} else {
				iter, _ = store.JobsByNamespace(ws, namespace, state.SortDefault)
			}
		...
		case structs.Allocs:
			...
			if wildcard(namespace) {
				iter, _ := store.Allocs(ws, state.SortDefault)
				iter = nsCapIterFilter(iter, aclObj)
			} else {
				iter, _ = store.AllocsByNamespace(ws, namespace)
			}
			m, t := getFuzzyMatchesImpl(iter, text, limitQuery, limitResults)
			mergeFuzzyMatches(matches, truncs, m, t)

		case structs.Variables:
			var iter memdb.TableResultIterator[*structs.VariableEncrypted]
			if wildcard(namespace) {
				iter, _ = store.Variables(ws)
				iter = nsCapIterFilter(iter, aclObj)
			} else {
				iter, _ = store.GetVariablesByNamespace(ws, namespace)
			}
			m, t := getFuzzyMatchesImpl(iter, text, limitQuery, limitResults)
			mergeFuzzyMatches(matches, truncs, m, t)
		...
		case structs.Nodes:
			...
			iter, _ := store.Nodes(ws)
			m, t := getFuzzyMatchesImpl(iter, text, limitQuery, limitResults)
			mergeFuzzyMatches(matches, truncs, m, t)

		case structs.NodePools:
			iter, _ := store.NodePools(ws, state.SortDefault)
			if !aclObj.IsManagement() {
				iter = memdb.NewTableFilterIterator(iter, nodePoolCapFilter(aclObj))
			}
			m, t := getFuzzyMatchesImpl(iter, text, limitQuery, limitResults)
		...
		case structs.Plugins:
			...
			iter, _ := store.CSIPlugins(ws)
			m, t := getFuzzyMatchesImpl(iter, text, limitQuery, limitResults)
			mergeFuzzyMatches(matches, truncs, m, t)

		case structs.Namespaces:
			iter, _ := store.Namespaces(ws)
			iter = nsCapIterFilter(iter, aclObj)
			m, t := getFuzzyMatchesImpl(iter, text, limitQuery, limitResults)
			mergeFuzzyMatches(matches, truncs, m, t)
