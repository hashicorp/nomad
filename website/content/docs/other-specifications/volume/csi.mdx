...
description: Learn about the Volume specification used to create and register CSI volumes to Nomad.
---

# CSI Volume Specification

The Nomad volume specification defines the schema for creating and registering
CSI volumes using the [`volume create`][] and [`volume register`][] commands and the
...
- `topology_request` <code>([TopologyRequest][topology_request]: nil)</code> -
  ...
  be accessible from in the case of volume creation, or the locations where the
  existing volume is accessible from in the case of volume registration.
