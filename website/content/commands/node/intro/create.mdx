...
Generates a new node introduction token. This token is used to authenticate a new Nomad client node
to the cluster. When possible, we recommend specifying the optional `-node-name` flag, so that the
token is scoped to a specific node.
...
- `-node-name`: The name of the node to which the introduction token will be scoped. If not
  specified, the value will be left empty.

- `-node-pool`: The node pool to which the introduction token will be scoped. If not specified, the
  value `default` will be used.
