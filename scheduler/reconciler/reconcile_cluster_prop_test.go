	...
	sharedSafetyProperties := func(t *rapid.T, ar *AllocReconciler, results *ReconcileResults, perTaskGroup map[string]map[string]int) {
		...
		for tgName, counts := range perTaskGroup {
			...
			if counts["expect_count"] == 0 || ar.jobState.Job.Stopped() {
				...
				// TODO(tgross): these reschedules don't result in placements,
				// which suggests we can optimize some of the downstream
				// handling if stopped/count=0
				//
				// TODO(tgross): it looks like we can produce other cases of this
				//
				// if tgUpdates.Disconnect >= 0 && tgUpdates.RescheduleNow > 0 {
				// 	must.LessEq(t, tgUpdates.Disconnect, tgUpdates.RescheduleNow,
				// 		tprintf("only reschedules should be for disconnects"))
				// }
