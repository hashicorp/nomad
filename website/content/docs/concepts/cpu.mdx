...
of its CPU. The two metrics associated with each Nomad node with regard to CPU
performance are its bandwidth (a.k.a _compute_) and the number of cores.

Modern CPUs may contain hetrogenous core types. Apple introduced the M1 CPU in
2020 which contains both _performance_ (P-Core) and _efficiency_ (E-Core) types.
Each core type operates a different base frequency. Intel introduced a similar
topology in its Raptor Lake chips in 2022. When fingerprinting the characteristics
...
do not support hyperthreading, wheras P-Cores do. As such a single physical
...
The amount of reserved CPU can be specified in bandwidth via `cpu`, e.g.
...
Or the amount of reserved CPU can be specified by specifing a specific set of
`cores` on which to disallow the scheduling of Nomad tasks. This capability is
...
Once the CPU is constrained by one of the above configurations, the node 
attribute `cpu.usablecompute` will indicate the total amount of CPU bandwdith
available for scheduling of Nomad tasks.

## Allocating CPU Resources

When scheduling a job a Task must specify how much CPU resource should be
allocated on behalf of the task. This can be done in terms of bandwidth in MHz
with the `cpu` attribute.
...
this option will restrict tasks from bursting above their CPU limit even
when there is idle capacity on the node. The tradeoff is consistency versus
utilization. A task with too few CPU resources may operate fine until another
task is placed on the node causing a reduction in available CPU bandwidth, which
could cause distruption for the underprovisioned task.

### CPU Environment Variables

To help tasks understand the resources available to them, Nomad sets environment
variables in the runtime environment of each task.


- `NOMAD_CPU_LIMIT` - The amount of CPU bandwidth allocated on behalf of the
task.

- `NOMAD_CPU_CORES` - The set of cores in [cpuset][] notation reserved for the task.
This value is only set if `resources.cores` is configured.
...
Any server that contains multiple CPU sockets or multiple RAM banks per CPU
socket will be characterized by the non-uniform access times involved in
...
Any OS process scheduled on core X where X belongs to  memory node M will take
longer to access any memory node that is not M.
...
in Mem 2 might take 300% longer. The extreme differences are due to various
physical hardware limitations. A core accessing memory in its own NUMA node is
optimal. Programs which perform a high throughput of reads or writes to/from
system memory will have their performance substantially hindered by not optimizing
their spatial locality iwth regard to the systems NUMA topology.

### SLIT tables

Any modern machine will define System Locality Distance Information (SLIT)
tables in its firmware. These tables are understood and made referenceable
by the Linux kernel. There are two key pieces of information provided by SLIT
tables
...
And the associated performance degredations are available via `numactl`
...
is occuring from a CPU that is part of the same NUMA node. A value of 20 for
example would indicate a 200% performance degredation, 30 for 300%, etc.

### node attributes

A Nomad client will fingerprint the machine's NUMA topology and export the
...
Starting in Nomad Enterprise 1.7, Nomad is capable of scheduling tasks in a way
that is optimized for the NUMA topology of a client node. A task may specify
a `numa` block indicating its NUMA optimization preference.
...
Currently there are three supported affinity options: `none`, `prefer`, and
`require`, each with their own advantages and tradeoffs.
