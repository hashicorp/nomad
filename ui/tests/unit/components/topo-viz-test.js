...
module('Unit | Component | TopoViz', function (hooks) {
  ...
  test('the topology object properly organizes a tree of datacenters > nodes > allocations', async function (assert) {
    ...
    assert.deepEqual(topoViz.topology.datacenters.map(item => get(item, 'name')), [
      'dc1',
      'dc2',
    ]);
    assert.deepEqual(topoViz.topology.datacenters[0].nodes.map(item => get(item, 'node')), [
      nodes[0],
      nodes[2],
    ]);
    assert.deepEqual(topoViz.topology.datacenters[1].nodes.map(item => get(item, 'node')), [
      nodes[1],
    ]);
    assert.deepEqual(
      topoViz.topology.datacenters[0].nodes[0].allocations.map(item => get(item, 'allocation')),
      node0Allocs,
    );
    assert.deepEqual(
      topoViz.topology.datacenters[1].nodes[0].allocations.map(item => get(item, 'allocation')),
      node1Allocs,
    );
    assert.deepEqual(
      topoViz.topology.datacenters[0].nodes[1].allocations.map(item => get(item, 'allocation')),
      node2Allocs,
    );
  });
  ...
  test('the topology object contains an allocation index keyed by jobId+taskGroupName', async function (assert) {
    ...
    Object.keys(topoViz.topology.allocationIndex).forEach((key) => {
      ...
      assert.deepEqual(
        topoViz.topology.allocationIndex[key].map(item => get(item, 'allocation')),
        allocations.filter(
