...
// SandboxVolumeChecker is a FeasibilityChecker which returns whether a node has
// the host volumes necessary to schedule a task group.
type SandboxVolumeChecker struct {
	ctx        Context
	volumeReqs []*structs.VolumeRequest
	sandboxes  map[string][]*structs.SandboxVolume // name -> count of claims
	// namespace     string
	// jobID         string
	// taskGroupName string
	//	unmetClaims   []*structs.TaskGroupSandboxVolumeClaim
}

// NewSandboxVolumeChecker creates a SandboxVolumeChecker from a set of volumes
func NewSandboxVolumeChecker(ctx Context) *SandboxVolumeChecker {
	hostVolumeChecker := &SandboxVolumeChecker{
		ctx: ctx,
		//	unmetClaims: []*structs.TaskGroupSandboxVolumeClaim{},
		sandboxes:  map[string][]*structs.SandboxVolume{},
		volumeReqs: []*structs.VolumeRequest{},
	}
...
// SetVolumes takes the volumes required by a task group and updates the checker.
func (h *SandboxVolumeChecker) SetVolumes(ns string, tg *structs.TaskGroup) {
	for _, req := range tg.Volumes {
		if req.Type == structs.VolumeTypeSandbox {
			iter, err := h.ctx.State().SandboxesByName(nil, ns, req.Source)
			if err != nil {
				h.ctx.Logger().Error("could not query sandboxes", "error", err)
			}
			sandboxForSrc := []*structs.SandboxVolume{}

			for obj := iter.Next(); obj != nil; obj = iter.Next() {
				sandbox := obj.(*structs.SandboxVolume)
				sandboxForSrc = append(sandboxForSrc, sandbox)
			}
			h.sandboxes[req.Source] = sandboxForSrc
		}
	}
}

func (h *SandboxVolumeChecker) Feasible(candidate *structs.Node) bool {
	if len(h.volumeReqs) == 0 {
		return true // fast path for no sandboxes
	}
...
// 	h.taskGroupName = taskGroupName
// 	h.volumeReqs = []*structs.VolumeRequest{}
// 	h.unmetClaims = []*structs.TaskGroupSandboxVolumeClaim{}

// 	storedClaims, _ := h.ctx.State().TaskGroupSandboxVolumeClaimsByFields(
// 		nil, state.TgvcSearchableFields{
// 			Namespace: ns,
// 			JobID:     jobID,
// 		})
...
// NEXT:
// 	for raw := storedClaims.Next(); raw != nil; raw = storedClaims.Next() {
// 		claim := raw.(*structs.TaskGroupSandboxVolumeClaim)

// 		alloc, err := h.ctx.State().AllocByID(nil, claim.AllocID)
// 		if err != nil || alloc == nil {
// 			continue
// 		}
// 		vol, err := h.ctx.State().SandboxVolumeByID(nil, ns, claim.VolumeID, false)
// 		if err != nil || vol == nil {
// 			continue
// 		}
...
// func proposedAllocMeetsClaim(
// 	proposed *structs.Allocation,
// 	claim *structs.TaskGroupSandboxVolumeClaim,
...
// 	h.ctx.Metrics().FilterNode(candidate, FilterConstraintSandboxVolumes)
// 	return false
// }
