...
- `wait` `(map: { min = "5s" max = "4m" })` - Defines the minimum and maximum amount of time to wait
  ...
  equal to the specified `min` value. The loop will not re-render the template until the `max` value duration
  has elapsed, or the underlying data has changed. This is useful to enable in systems where network
