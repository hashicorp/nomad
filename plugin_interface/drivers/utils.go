...
func ResourcesFromProto(pb *proto.Resources) *Resources {
	...
	if pb.AllocatedResources != nil {
		r.NomadResources = &AllocatedTaskResources{}
		...
		for _, network := range pb.AllocatedResources.Networks {
			...
			for _, port := range network.ReservedPorts {
				n.ReservedPorts = append(n.ReservedPorts, Port{
					Label: port.Label,
					Value: int(port.Value),
				})
			}
			for _, port := range network.DynamicPorts {
				n.DynamicPorts = append(n.DynamicPorts, Port{
					Label: port.Label,
					Value: int(port.Value),
				})
			}
	...
	if pb.Ports != nil {
		ports := AllocatedPorts(make([]AllocatedPortMapping, len(pb.Ports)))
		for i, port := range pb.Ports {
			ports[i] = AllocatedPortMapping{
				Label:  port.Label,
				Value:  int(port.Value),
				To:     int(port.To),
				HostIP: port.HostIp,
