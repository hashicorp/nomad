...
func (s *GenericStack) Select(tg *structs.TaskGroup, options *SelectOptions) *RankedNode {
	...
	if options != nil && len(options.PreferredNodes) > 0 {
		...
		originalFastPathNodes := options.FastPathNodes
		...
		optionsNew.FastPathNodes = originalFastPathNodes
		s.source.SetNodes(originalNodes)
		return s.Select(tg, &optionsNew)
	}

	// If the preferred nodes weren't selected, we'll try the fast-path nodes
	// when we know there's only a small number of nodes available that can pass
