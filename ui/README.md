...
- `pnpm start` runs the UI at [http://localhost:4200/ui](http://localhost:4200/ui).
- `pnpm start:proxy` proxies API traffic to Nomad and serves the UI at [http://localhost:4646/ui](http://localhost:4646/ui).
...
Use `pnpm start:proxy` to proxy the UI to Nomad at `http://127.0.0.1:4646` while serving the app at `http://localhost:4646/ui`. If you are running Nomad at a different address, override the proxy target when running ember serve: `USE_MIRAGE=false ember serve --proxy http://newlocation:1111`.
