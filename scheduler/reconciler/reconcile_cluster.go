...
func (a *AllocReconciler) computeGroup(group string, all allocSet) (*ReconcileResults, bool) {
	...
	// Determine what set of migrating allocation need to be rescheduled
...
func (a *AllocReconciler) computeMigrations(migrate allocSet, isCanarying bool,
	...
	for _, alloc := range migrate.nameOrder() {
		...
		// Do not create a placement if this is a batch job allocation. If
		// the allocation is being drained it will not be replaced. If the
		// allocation is being stopped via the alloc endpoint, then it will
