...
As of Nomad 1.x.x (**TODO**: What version would this land in?) `retry_join` accepts
Cloud Auto-join configurations in [go-netaddrs](https://github.com/hashicorp/go-netaddrs) 
format. See [Cloud Auto-join](#cloud-auto-join) for more information.
...
Executables may be referenced by absolute file system path or by any program
name discoverable by `os/exec`'s [LookPath](https://pkg.go.dev/os/exec#LookPath)
function.
...
deliminted by piping its output into `tr`.

Output of an exmaple `go-netaddrs` executable
```
"1.1.1.1 2.2.2.2"
```
