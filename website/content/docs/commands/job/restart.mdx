...
run. Groups and tasks can also target allocations.  When you define both groups and tasks, Nomad restarts only the tasks for the allocations of those groups.
...
- `-reschedule`: If set, Nomad stops and migrates allocations instead of
  ...
  [`max_parallel`][], are not considered. This option cannot be used
  with `-task`. You may only migrate jobs of type `batch`, `service`, and `system`.
Note that despite the name of this flag, Nomad migrates but does not reschedule allocations, so Nomad ignores the `reschedule` block.
