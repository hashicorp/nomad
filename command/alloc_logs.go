...
Logs Specific Options:
  ...
  -f
    ...
    rather to wait for additional output. When supplied with no other flag,
    both stdout and stderr logs will be followed.
...
func (l *AllocLogsCommand) tailMultipleFiles(client *api.Client, alloc *api.Allocation) error {
	...
	for {
		...
		case stdoutErr := <-stdoutErrCh:
			return fmt.Errorf("recived error from stdout log stream: %v", stdoutErr)
		case stdoutFrame := <-stdoutFrames:
			l.Ui.Output(string(stdoutFrame.Data[:]))
		case stderrErr := <-stderrErrCh:
			return fmt.Errorf("recived error from stderr log stream: %v", stderrErr)
		case stderrFrame := <-stderrFrames:
			l.Ui.Warn(string(stderrFrame.Data[:]))
		}
	}
}
