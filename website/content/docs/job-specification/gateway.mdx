...
- `tls_min_version` `(string: optional)` - Set this configuration to override
  the default min tls version supported by the gateway. Details for supported versions
  see [Consul TLSMinVersion](https://www.consul.io/docs/connect/config-entries/ingress-gateway#tlsminversion).

- `tls_max_version` `(string: optional)` - Set this configuration to override
   the default max tls version supported by the gateway. Details for supported versions
   see [Consul TLSMaxVersion](https://www.consul.io/docs/connect/config-entries/ingress-gateway#tlsmaxversion).

- `cipher_suites` `(array<string>: optional)` - Set this configuration to override
   the default list of TLS cipher suites for the gateway's listeners. Details for supported cipher suites
   see [Consul CipherSuites](https://www.consul.io/docs/connect/config-entries/ingress-gateway#ciphersuites).
