...
The information below should be used to download the task driver for use on your Nomad client
host.

<Tabs>
  <Tab heading="Manual installation" group="manual">

    You can download a [precompiled binary](https://releases.hashicorp.com/nomad-driver-virt/) and
    verify the binary using the available SHA-256 sums. After downloading nomad-driver-virt driver,
    unzip the package. Make sure that the `nomad-driver-virt` binary is available on your
