...
func TestHTTP_AgentSelf(t *testing.T) {
	...
	httpTest(t, nil, func(s *TestAgent) {
		...
		require.Empty(self.Config.Vaults[structs.VaultDefaultCluster].Token)

		// Assign a Vault token and require it is redacted.
		s.Config.Vaults[structs.VaultDefaultCluster].Token = "badc0deb-adc0-deba-dc0d-ebadc0debadc"
		respW = httptest.NewRecorder()
		obj, err = s.Server.AgentSelfRequest(respW, req)
		require.NoError(err)
		self = obj.(agentSelf)
		require.Equal("<redacted>", self.Config.Vaults[structs.VaultDefaultCluster].Token)
		...
		require.Empty(self.Config.Consuls[structs.ConsulDefaultCluster].Token)

		// Assign a Consul token and require it is redacted.
		s.Config.Consuls[structs.ConsulDefaultCluster].Token = "badc0deb-adc0-deba-dc0d-ebadc0debadc"
		respW = httptest.NewRecorder()
		obj, err = s.Server.AgentSelfRequest(respW, req)
		require.NoError(err)
		self = obj.(agentSelf)
		require.Equal("<redacted>", self.Config.Consuls[structs.ConsulDefaultCluster].Token)
