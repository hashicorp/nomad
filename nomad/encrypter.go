...
func (krr *KeyringReplicator) replicateKey(ctx context.Context, wrappedKeys *structs.RootKey) error {
	...
	isClusterUpgraded := krr.srv.peerPartsCache.ServersMeetMinimumVersion(
...
func (krr *KeyringReplicator) getAllPeers() []*peer.Parts {
	krr.srv.peerLock.RLock()
	defer krr.srv.peerLock.RUnlock()
	peers := make([]*peer.Parts, 0, len(krr.srv.localPeers))
