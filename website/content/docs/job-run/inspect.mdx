...
`nomad alloc status` command will give the reason why.
...
Clicking on a job from the list on the **Jobs** page in the web UI takes you to
the overview page for that job. The overview page has several linked pages at
the top for viewing information about the running job.
...
allocations, any placement failures, and associated task groups.
- **Definition**: A page displaying the job definition in JSON.
- **Versions**: A page with a timeline view of every version of the job.
- **Deployments**: A page with health information about job deployments as well
as associated task groups and allocations.
- **Allocations**: A page with a list of associated allocations.
- **Evaluations**: A page with a list of associated evaluations.
- **Services**: A page with a list of services defined in the jobspec registered
for service discovery and any associated allocations.
