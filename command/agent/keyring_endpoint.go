...
// TODO(schmichael) should this be based on the rotation threshold?
...
func (s *HTTPServer) JWKSRequest(resp http.ResponseWriter, req *http.Request) (any, error) {
	...
	if newestKey > 0 && rpcReply.RotationThreshold > 0 {
		...
		maxAge := exp.Sub(time.Now())

		// Subtract some jitter
		//TODO(schmichael) we can probably jitter a lot more
		maxAge -= helper.RandomStagger(jwksMinMaxAge)

		// Keys should be published well before they're used, so ensure a minimum
		// amount of caching is always used.
		if maxAge < jwksMinMaxAge {
			maxAge = jwksMinMaxAge + helper.RandomStagger(jwksMinMaxAge)
		}
