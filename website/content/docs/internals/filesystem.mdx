...
- **alloc/**: This directory is shared across all tasks in an allocation and
  ...
  in a task group can read and write to the `alloc/` directory, but the path
  may differ depending on the task driver's [filesystem isolation mode]. Tasks
  with driver isolation of [`none`](#none-isolation) may reference this
  directory using the `NOMAD_ALLOC_DIR` environment variable or a relative path
  that traverses up into the allocation working directory (`../alloc/`). Other
  task drivers will have this directory provided within their working
  directory, and may access it via the `alloc/` path, such as with
  [`image`](#image-isolation) and [`chroot`](#chroot-isolation) isolations.
  Within the `alloc/` directory are three standard directories:
