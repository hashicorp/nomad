...
- `user` `(string: <varies>)` - Specifies the user that will run the task.
  ...
  On the Windows platform, in case Nomad is running as [system service][service],
  for the [`raw_exec`][raw_exec] driver less privileged service user can be
  specified (example: `NT AUTHORITY\LocalService`, `NT AUTHORITY\NetworkService`).
