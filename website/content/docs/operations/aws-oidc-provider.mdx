---
layout: docs
page_title: Nomad OpenID Connect with AWS Identity Provider
description: |-
...
- An AWS account with the necessary permissions to create IAM roles, policies, hosted zones, and
certificates.
- Terraform installed locally and configured to communicate with AWS.

## Set up and configure the Nomad cluster domain

In order to use Nomad as an identity provider, you need to have a trusted SSL certificate for the
...
Create a jobspec file named `s3-upload.nomad.hcl`, add the following configuration to it, and
save the file.
