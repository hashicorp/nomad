---
layout: docs
page_title: Authenticate Nomad jobs with AWS IAM Identity Provider
description: |-
  Integrate Nomad as an OpenID Connect (OIDC) provider with AWS IAM Identity.
---

# Authenticate Nomad jobs with AWS IAM Identity Provider

This page describes how to integrate Nomad with AWS IAM Identity Provider as an OpenID Connect
(OIDC). You can configure Nomad jobs to present a JWT token that the Nomad server authenticates 
with AWS IAM before it executes a task.

## Prerequisites

To integrate Nomad and the AWS IAM Identity Provider, you need a running Nomad cluster that
...
The process to integrate Nomad with the AWS IAM provider consists of the following steps:
...
1. Create a jobspec file that uses AWS OIDC and then run it to verify your configuration.
...
domain used by the cluster. You will use AWS Certificate Manager (ACM) in this example.
...
page.
- [`<LB_TARGET_GROUP_ARN>`](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/lb_listener#target_group_arn): The AWS resource name of the target group for the load balancer to route traffic to.
...
This configuration requires the following information:
- [`<ROUTE53_ZONE_ID>`](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/route53_record#zone_id):
