...
- `gc_disk_usage_threshold` `(float: 80)` - Specifies the disk usage percent
  ...
  that terminal allocations will be garbage collected immediately if garbage
...
- `gc_max_allocs` `(int: 50)` - Specifies the maximum number of allocations
  ...
  allocations to be GC'd. Note that terminal allocations will be garbage
  collected immediately if garbage collected on the server.
