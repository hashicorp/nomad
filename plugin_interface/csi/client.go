...
func newGrpcConn(addr string, logger hclog.Logger) (*grpc.ClientConn, error) {
	...
	conn, err := grpc.DialContext(
		...
		grpc.WithBlock(),
		grpc.WithInsecure(),
		grpc.WithUnaryInterceptor(helper.UnaryClientInterceptor(logger)),
		grpc.WithStreamInterceptor(helper.StreamClientInterceptor(logger)),
	...
	if err != nil {
		...
		case codes.NotFound:
			...
			err = fmt.Errorf("%w: volume %q or node %q could not be found: %v",
				ErrCSIClientRPCIgnorable, req.ExternalID, req.NodeID, err)
		case codes.Internal:
			err = fmt.Errorf("controller plugin returned an internal error, check the plugin allocation logs for more information: %v", err)
		}
		return nil, err
	...
	if err != nil {
		...
		case codes.NotFound:
			err = fmt.Errorf("%w: volume %q could not be found: %v",
				ErrCSIClientRPCIgnorable, volumeID, err)
		case codes.Internal:
			err = fmt.Errorf("node plugin returned an internal error, check the plugin allocation logs for more information: %v", err)
		}
	}
	...
	if err != nil {
		...
		case codes.NotFound:
			return nil, fmt.Errorf("%w: volume %q could not be found: %v",
				ErrCSIClientRPCIgnorable, req.ExternalVolumeID, err)
		case codes.FailedPrecondition:
