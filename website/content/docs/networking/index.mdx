...
group "..." {
  ...
  task "..." {
    ...
    config {
      # This conclicts with the group-level network.mode configuration and
      # should not be used.
      network_mode = "bridge"
      # ...
    }
...
Since the group `network.mode` is `bridge`, Nomad created the pause cointainer
to establish a shared network namespace for all tasks, but setting the
task-level `network_mode` to `bridge` creates a different namespace for the
other task.
