...
func TestExecStreamingInputIsInSync(t *testing.T) {
	cases := []struct {
		...
		{
			"stdin_data",
			api.ExecStreamingInput{Stdin: &api.FileOperation{Data: []byte("hello there")}},
		},
		{
			"stdin_close",
			api.ExecStreamingInput{Stdin: &api.FileOperation{Close: true}},
		},
		{
			"tty_size",
...
func TestExecStreamingOutputIsInSync(t *testing.T) {
	cases := []struct {
		...
		{
			"stdout_data",
			api.ExecStreamingOutput{Stdout: &api.FileOperation{Data: []byte("hello there")}},
		},
		{
			"stdout_close",
			api.ExecStreamingOutput{Stdout: &api.FileOperation{Close: true}},
		},
		{
			"stderr_data",
			api.ExecStreamingOutput{Stderr: &api.FileOperation{Data: []byte("hello there")}},
		},
		{
			"stderr_close",
			api.ExecStreamingOutput{Stderr: &api.FileOperation{Close: true}},
		},
		{
			"exited",
