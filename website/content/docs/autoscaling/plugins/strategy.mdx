...
and returns a scaling action whose count to set the same value. This strategy is
...
the APM query result as the scaling count. This allows the Nomad Autoscaler to
be integrated into systems where the scaling computation is done externally like
an edge router, or easily derived from a query such as running one allocation per
item in a queue.
