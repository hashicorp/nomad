...
`iso`), and is currently invoked with `qemu-system-ARCH`. A custom binary path
can be configured with [`binary_path`](#binary_path).

The driver requires the image to be accessible from the Nomad client via the
[`artifact` downloader](/docs/job-specification/artifact) or the configured
...
task "webservice" {
  ...
  config {
    # ARCH in the binary_path will be replaced with the host system's arch
    binary_path       = "/path/to/qemu-system-ARCH"
...
- `binary_path` - The path to the QEMU binary to be used. If `binary_path` does
  not contain a slash, the value will be treated as the binary's name and
  looked up on Nomad's `$PATH`. The QEMU architecture name of the host (e.g.
  `x86_64`, `arm`) can be automatically interpolated with `ARCH`. Defaults
  to `qemu-system-ARCH`.
