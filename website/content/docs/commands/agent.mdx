...
If you are running Nomad on Linux, you'll need to run client agents as root (or
...
- `-acl-bootstrap-token`: Equivalent to the ACL [bootstrap_token][] config.

  <Note>
  Once the cluster's ACL subsystem has been bootstrapped, this value is no longer
  consulted and you should remove this flag from the configuration. Nomad
  writes a log event on startup if this flag is set and no longer necessary.
  </Note>
...
[bootstrap_token]: /nomad/docs/configuration/acl#bootstrap_token
