...
module('Acceptance | actions', function (hooks) {
  ...
  hooks.beforeEach(async function () {
    ...
    const actionsJob = this.server.create('job', {
    ...
    const actionsGroup = this.server.create('task-group', {
      jobId: actionsJob.id,
    ...
    this.server.schema.allocations
      .find(el => el.taskGroup === actionsGroup.name)
      .update({
  ...
  test('Actions show up on the Job Index page, permissions allowing', async function (assert) {
    ...
    let clientReaderToken = this.server.create('token', {
      type: 'client',
      name: "N. O'DeReader",
    });

    const allocExecPolicy = this.server.create('policy', {
      id: 'alloc-exec',
      rules: `
        namespace "*" {
          policy = "read"
    ...
    let allocExecToken = this.server.create('token', {
      type: 'client',
  ...
  test('Running actions and notifications', async function (assert) {
    ...
    assert.deepEqual(Actions.titleActions.expandedValue, 'true');
    assert.deepEqual(
    ...
    assert.deepEqual(Actions.titleActions.expandedValue, 'false');
  ...
  test('Running actions from a task row', async function (assert) {
    ...
    const job = this.server.schema.jobs.find('actionable-job');
    const numberOfTaskRows = this.server.schema.allocations
      .all()
