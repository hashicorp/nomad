...
Nomad serves the web UI alongside the API. The web UI can be viewed in a web
browser with the [`nomad ui` command](/nomad/commands/ui) or by using the
cluster address and navigating to the `/ui` path. 
...
The first page shown when accessing the web UI is a listing of all the Jobs
for the default namespace. Clicking on a Job will take you to the Job details
page.

### View a job directly from the CLI

Directly open the details page for a Job by providing the job name to the
`nomad ui` command.
...
### View an allocation directly from the CLI

Directly open the details page for an allocation by providing the allocation ID
