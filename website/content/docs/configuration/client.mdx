...
- `disable_artifact_inspection` `(bool: false)` - Specifies whether artifact
  inspection for sandbox escapes should be disabled. If the platform supports
  filesystem isolation, and it is not disabled, artifact inspection will not
  be performed regardless of this value.
