...
If you access the web UI without specifying an ACL token, you will be granted
the permissions of the anonymous policy. An ACL token can be set in the web UI or with the CLI.

### Provide a token in the web UI

Open the web UI in your web browser by using cluster address and the `/ui` path.
Click on the user icon in the top-right of the page to open the **Settings**
page. Enter your ACL token in the **Secret ID** field and click the
**Sign in with secret** button. You can also open this page directly by
