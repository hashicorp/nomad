...
- `volumes` stanza:
  ...
  - `selinuxlabel` - Allows the operator to set a SELinux label to the
    allocation bind-mounts to containers. When `selinux-enabled` is `true` on
    the Docker daemon and SELinux is enforced, this should be set to `Z` (or
    `z`) in order to have access to `/local` and `/secrets`.
