...
Since the Nomad UI provides a much richer user experience than the CLI, additional API endpoints `/v1/agent` `/v1/node` are used when rendering pages.  Adding the rules below to your ACL when utilizing the Nomad UI is recommended.
