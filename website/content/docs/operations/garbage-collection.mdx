...
- Periodic timer
  ...
  The AllocGarbageCollector launches a ticker based on the configured
...
These settings govern allocation GC behavior on each client node. Notably,
**there is no time-based retention setting for allocations** – unlike jobs or
evals, you cannot specify “keep allocations for X hours before GC.” As soon as
an alloc is terminal, it becomes eligible for cleanup if the above thresholds
demand it. This is a known design choice. The client doesn’t wait a fixed time –
it will remove the alloc as soon as it needs to for resource reasons.)
