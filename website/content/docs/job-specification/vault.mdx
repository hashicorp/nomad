...
- `entity_alias` `(string: "")` - Specifies the entity alias to use when creating
  Vault tokens in the task. This allows multiple tokens to be created as a single
  Vault client. If given, this value overrides the "entity_alias" value found in the
  [Nomad server vault configuration](/docs/configuration/vault#nomad-server).
