...
```plaintext
nomad job action [options] <job_action_name>
```
...
- `-task`: Specifies the task within the job where the action is defined. This
  must correspond to a task in the job's specification, and is required if no
  `alloc` option is provided.

- `-group`: Specifies the task group within the job. This is needed when
  selecting a random allocation from a specific task, and is required if no
  `alloc` option is provided.
