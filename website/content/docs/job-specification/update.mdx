...
- `canary` `(int: 0)` - Specifies that changes to the job that would result in
  ...
  In system jobs, `canary` setting indicates the percentage of eligible nodes to
  which allocations will be deployed. System job updates are always destructive
  and we do not support more than one allocation per node, so effectively
  setting `canary` to a positive integer means this amount of eligible nodes
  will get a new version of the job. Setting `canary` to 100 updates the job on
  all nodes. Percentage of nodes is always rounded up to the nearest integer.
