...
- `allow_token_expiration` `(bool: false)` - Specifies that Nomad clients should
  ...
  exhausted, at which point the task will fail. This field is valid only when
  Nomad has been configured to use [Workload Identity with Vault][] and will be
  ignored when using the [legacy Vault authentication workflow][].
