...
description: The /acl/identity/ endpoint is used to to create client introduction identity tokens.
---

# ACL Identities HTTP API

The `/acl/identity` endpoint is used to create client introduction identity tokens.
...
- `NodeName` `(string: <optional>)` - The name of the node to which the introduction token will be
  scoped. If not specified, the value will be left empty.

- `NodePool` `(string: <optional>)` - The node pool to which the introduction token will be scoped.
  If not specified, the value `default` will be used.

- `TTL` `(string: <optional>)` - The TTL to apply to the introduction token. If not specified, the
  server configured default value will be used.
