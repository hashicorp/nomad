...
func initialize() {
	...
	if err != nil {
		panic("nomad.freeport: ephemeral port range detection failed: " + err.Error())
	}
	if effectiveMaxBlocks < 0 {
		panic("nomad.freeport: no blocks of ports available outside of ephemeral range")
	}
	if lowPort+effectiveMaxBlocks*blockSize > 65535 {
		panic("nomad.freeport: block size too big or too many blocks requested")
	}
...
func alloc() (int, net.Listener) {
	...
	panic("nomad.freeport: cannot allocate port block")
}
...
func Take(n int) (ports []int, err error) {
	if n <= 0 {
		return nil, fmt.Errorf("nomad.freeport: cannot take %d ports", n)
	}
	...
	for len(ports) < n {
		for freePorts.Len() == 0 {
			if total == 0 {
				return nil, fmt.Errorf("nomad.freeport: impossible to satisfy request; there are no actual free ports in the block anymore")
			}
			condNotEmpty.Wait()
		}
...
func logf(severity string, format string, a ...interface{}) {
	_, _ = fmt.Fprintf(os.Stderr, "["+severity+"] nomad.freeport: "+format+"\n", a...)
}
