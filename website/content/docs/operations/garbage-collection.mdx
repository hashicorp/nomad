...
For example, garbage collecting a job also causes
Nomad to drop all of that job's remaining evaluations, deployments, and
allocation records from the state. Nomad garbage collects those
objects, either as part of the job garbage collection process or by their own
garbage collection processes running immediately after. Nomad only garbage
collects objects after they are terminal for at least the specified time
threshold and no longer needed for future scheduling decisions.
...
| **Plugin** | 5 minutes | [`csi_plugin_gc_threshold`](/nomad/docs/configuration/server#csi_plugin_gc_threshold)<br/>Default: 1 hour |
...
Manually triggering garbage collection by running the `nomad system gc` command
instructs the server leader to ignore thresholds and immediately purge all
terminal objects.
...
- User-initiated server garbage collection
  ...
  When you initiate garbage collection on the server by running the `nomad
  system gc` command, the clients collect all eligible allocations, bypassing
  normal thresholds. Garbage collection first destroys all terminal allocations.
  Then the process pops every remaining finished allocation and deletes it
  within reason.
...
A task driver does not have its own garbage collection process. Instead, the client garbage collection process triggers a task driver to clean up
its resources when the allocation is terminal.

When the task has configured restart attempts and the task fails, the Nomad
client attempts an in-place task restart within the same allocation. The task
driver starts a new process or container for the task. If the task continues to
fail and exceeds the configured restart attempts, Nomad marks the allocation as
terminal. The client garbage collection process instructs the task driver to
clean up its resources, such as a Docker container or cgroups. The garbage
collection process waits for the task driver to signal completion before
deleting the allocation. If a task driver fails to clean up properly, Nomad logs
errors but continues the garbage collection process. Task driver cleanup failure
issues can influence when the allocation truly frees up. For instance, if
volumes are not detached, disk space might not be fully reclaimed until fixed.
