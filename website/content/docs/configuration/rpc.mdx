...
description: >-
   Configure Nomad's RPC Sessions multiplex protocol (rpc) system behavior in the `rpc` block of a Nomad agent configuration. Modify RPC multiplex session configuration, change tcp backlog, and keepalive interval, open socket timeout's.
---
...
This page provides reference information for configuring Nomad's RPC Session system behavior in the `rpc` block of a Nomad agent configuration. Modify RPC multiplex session configuration, change tcp backlog, and keepalive interval, open socket timeout's.
