...
* The `constraint` fields can be updated but are ignored after the volume is
  created.
* The `capability` blocks can be added or removed, but only if the capability is
  not currently in use by a mounted volume.

The `name`, `type`, and `node_id` fields cannot be updated. The `node_pool`
field can only be changed from empty to the node pool that matches the `node_id`
field.
