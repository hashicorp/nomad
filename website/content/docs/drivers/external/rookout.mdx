...
1. Change to the cloned dir: `cd rookout-nomad-driver`
1. Create a plugin dir (unless you already have one for your Nomad installation): `mkdir plugins`
1. Copy the corresponding binary from the `bin` folder, for example: `cp ./bin/java-rookout-linux-x64 plugins/java-rookout`
1. Make sure that the driver has execution permission: `chmod +x ./plugins/java-rookout`
1. When running the Nomad agent, be sure to set the plugins dir accordingly, for example: `sudo nomad agent -dev -plugin-dir=/home/user/rookout-nomad-driver/plugins`
