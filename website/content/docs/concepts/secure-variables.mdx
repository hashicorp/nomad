...
Most Nomad workloads need access to config values or secrets. Nomad provides a
`template` block to run render such configuration on behalf of tasks, but has
left the role of storing that configuration to external services such as
...
delete secrets via the command line or in the Nomad web UI.
...
init -short` will automatically have access to Secure Variables as if it had the
following policy:
