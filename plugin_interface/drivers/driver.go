...
import (
	...
	"github.com/hashicorp/nomad/plugin-interface/drivers/filesystem"
...
type DriverPlugin interface {
	...
	TaskStats(ctx context.Context, taskID string, interval time.Duration) (<-chan *TaskResourceUsage, error)
...
func (tc *TaskConfig) TaskDir() *filesystem.TaskDir {
	taskDir := filepath.Join(tc.AllocDir, tc.Name)
	return &filesystem.TaskDir{
		Dir:            taskDir,
		SharedAllocDir: filepath.Join(tc.AllocDir, filesystem.SharedAllocName),
		LogDir:         filepath.Join(tc.AllocDir, filesystem.SharedAllocName, filesystem.LogDirName),
		SharedTaskDir:  filepath.Join(taskDir, filesystem.SharedAllocName),
		LocalDir:       filepath.Join(taskDir, filesystem.TaskLocal),
		SecretsDir:     filepath.Join(taskDir, filesystem.TaskSecrets),
	}
}
...
type MemoryResources = AllocatedMemoryResources

type Resources struct {
	NomadResources *AllocatedTaskResources
...
func (r *Resources) Copy() *Resources {
	...
	if r.Ports != nil {
		ports := AllocatedPorts(append(make([]AllocatedPortMapping, 0, len(*r.Ports)), *r.Ports...))
		res.Ports = &ports
	}
	return res
}
