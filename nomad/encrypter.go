...
func (e *Encrypter) Encrypt(cleartext []byte) ([]byte, string, error) {
	...
	bi, err := cs.wrapper.Encrypt(context.Background(), cleartext, kms.WithAad([]byte(keyID)))
	if err != nil {
...
func (e *Encrypter) Decrypt(ciphertext []byte, keyID string) ([]byte, error) {
	...
	return ks.wrapper.Decrypt(ctx, bi, kms.WithAad([]byte(keyID)))
}
