...
[`max_dynamic_port`] that has not been allocated yet and creates a port mapping
...
plugin][cni_bridge]. Before using this mode you must first [install the CNI
plugins][cni_install] into your clients.
...
Docker bridge instead of using the one create by Nomad and, by default, each
...
allocation. The Docker task driver has its own network mode configuration that
defaults to `bridge` (`nat` on Windows).
...
[Docker Compose][docker_compose] allows you to run and manage multiple
containers using units called services.
...
pattern, Docker Compose includes an [internal DNS services][docker_dns] and
load balancer.

[Docker Swarm][docker_swarm] follows a similar pattern but uses an
[`overlay`][docker_overlay] network across multiple hosts instead of the
default `bridge` from Compose.
...
[cni_install]: /docs/integrations/consul-connect#cni-plugins
