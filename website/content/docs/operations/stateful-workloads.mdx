...
---
By default, HashiCorp Nomad's allocation storage is ephemeral. Nomad can discard
it during new deployments, when rescheduling jobs, or if it loses a client. This
is undesirable when running persistent workloads such as databases.
...
At the end of this article, we provide a [storage
comparison](#storage-comparison) table with the advantages, disadvantages, and
the ideal use-case for each storage option. This guidance will help you
understand and choose the best storage option for your Nomad allocation storage.

This document addresses traditional storage you can attach as a filesystem and
does not cover object storage such as AWS S3 or MinIO. 
...
loss. But, it relies on network performance, can have higher latency, be
lower-throughput, and need more maintenance.
...
Usually they're comprised of local disks (physically attached and very
performant, but not very persistent), network attached block devices (performant
and very persistent, but usually only attachable to one instance at a time), and
networked shared storage (less performant, but can be accessed from multiple
instances). 
...
- **Controller**: Communicates with the storage provider to manage the volume
	lifecycle. - **Node**: Runs on all Nomad clients and handles all local
	operations (for example, mounting/unmounting volumes in allocations). The
	node must be `privileged` to perform those operations. - **Monolithic**:
	Combines both the above roles.
...
evolving due to their dynamic nature - for instance, new workloads that
frequently need persistent storage added or removed. However, they present some
challenges in terms of maintenance - most notably, they need to run constantly,
be configured (including authentication and connectivity to the storage
platform), and updated to keep track with new features and bug fixes and keep
compatibility with the underlying storage platform. They also introduce a couple
of moving parts, can be difficult to troubleshoot, and have a complex security
profile (due to needing to run as `privileged` containers in order to be able to
mount volumes).
...
[personnas](/nomad/docs/concepts/security#personas) to configure and consume
...
To learn more about hard mounts and soft mounts, visit Red Hat's [NFS
...
In this document, you learned about the different persistent storage options for
Nomad.

To learn more about Nomad and the topics covered in this article, visit the
following resources:
### Allocations
- Learn how to [manage your allocations and its
  storage](/nomad/tutorials/manage-clusters)
...
- [Hetzner
  CSI](https://github.com/hetznercloud/csi-driver/blob/main/docs/nomad/README.md)
