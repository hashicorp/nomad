...
1. Create a Vault token role with the configuration described below.

1. Configure Nomad to use the created token role.

1. Give Nomad servers a periodic token with the "nomad-server" policy created
   above.
