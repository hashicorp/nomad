...
func (s *HTTPServer) EvalSpecificRequest(resp http.ResponseWriter, req *http.Request) (interface{}, error) {
	...
	case strings.HasSuffix(path, "/history"):
		evalID := strings.TrimSuffix(path, "history")
		return s.evalHistory(resp, req, evalID)
	default:
...
// TODO: why does it return any type?
func (s *HTTPServer) evalHistory(resp http.ResponseWriter, req *http.Request, evalID string) (interface{}, error) {
	if req.Method != "GET" {
		return nil, CodedError(405, ErrInvalidMethod)
	}
	...
	var out structs.EvalHistoryResponse
	if err := s.agent.RPC("Eval.History", &args, &out); err != nil {
		return nil, err
	}

	setMeta(resp, &out.QueryMeta)
	if out.EvalHistory == nil {
		out.EvalHistory = make(map[string]*structs.Evaluation)
	}
	return out.EvalHistory, nil
}
