...
layout: docs
page_title: Client
description: Nomad client requirements, capabilities, and configuration options.
---

# Client

The `virt` task driver is not built into Nomad. It must be [downloaded][releases] onto the client
host in the configured plugin directory. The client host must also be capable of running hardware
accelerated KVM virtual machines. This can be checked via the [kvm-ok][] command.
...
The following command can be used to install the package requirements:
...
The default plugin configuration is shown below with a description of each option.
...
- `data_dir` - (default: `"/var/lib/virt"`) - a directory where VM configuration files and
...
When installed, the `virt` plugin provides the following node attributes which can be used as
