...
// COMPAT 0.11: Currently, all the tests run for 2 cases:
// 1) Allow plan optimization
// 2) Not allowing plan optimization
// The code for not allowing plan optimizations is in place to allow for safer cluster upgrades,
// and backwards compatibility with the existing raft logs. The older code will be removed later,
// and these tests should then no longer include the testing for case (2)
...
func TestSystemSched_JobRegister_StickyAllocs(t *testing.T) {
	for _, allowPlanOptimization := range []bool{true, false} {
		t.Run(IsPlanOptimizedStr(allowPlanOptimization), func(t *testing.T) {
			...
			noErr(t, h.State.UpsertEvals(h.NextIndex(), []*structs.Evaluation{eval}))
			h1 := NewHarnessWithState(t, h.State, allowPlanOptimization)
	...
	for _, allowPlanOptimization := range []bool{true, false} {
		t.Run(IsPlanOptimizedStr(allowPlanOptimization), func(t *testing.T) {
			...
			for _, allocList := range newPlan.NodePreemptions {
				require.Len(allocList, 1)
				alloc, err := h.State.AllocByID(nil, allocList[0].ID)
				noErr(t, err)
				require.Equal(svcJob.ID, alloc.JobID)
			}
...
func TestSystemSched_NodeDown(t *testing.T) {
	for _, allowPlanOptimization := range []bool{true, false} {
		t.Run(IsPlanOptimizedStr(allowPlanOptimization), func(t *testing.T) {
			...
			if p := planned[0]; p.DesiredDescription != allocNodeTainted {
...
func TestSystemSched_NodeDrain(t *testing.T) {
	for _, allowPlanOptimization := range []bool{true, false} {
		t.Run(IsPlanOptimizedStr(allowPlanOptimization), func(t *testing.T) {
			...
			// Ensure the allocations is stopped
			if planned[0].DesiredDescription != allocNodeTainted {
				t.Fatalf("bad: %#v", planned[0])
			}
...
func TestSystemSched_Preemption(t *testing.T) {
	for _, allowPlanOptimization := range []bool{true, false} {
		t.Run(IsPlanOptimizedStr(allowPlanOptimization), func(t *testing.T) {
			...
			expectedPreemptedAllocIDs := []string{alloc1.ID, alloc2.ID, alloc3.ID}
			...
			for _, alloc := range preemptedAllocs {
				require.Contains(expectedPreemptedAllocIDs, alloc.ID)
			}
			// Look up the preempted allocs by job ID
			ws = memdb.NewWatchSet()

			for _, allocID := range expectedPreemptedAllocIDs {
				evictedAlloc, err := h.State.AllocByID(ws, allocID)
				noErr(t, err)
				require.Equal(structs.AllocDesiredStatusEvict, evictedAlloc.DesiredStatus)
				require.Equal(fmt.Sprintf("Preempted by alloc ID %v", preemptingAllocId), evictedAlloc.DesiredDescription)
			}
