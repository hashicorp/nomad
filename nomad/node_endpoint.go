...
func (n *Node) GetAllocs(args *structs.NodeSpecificRequest,
	...
	opts := blockingOptions{
		...
		queryMeta: &reply.QueryMeta,
		run: func(ws memdb.WatchSet, state *state.StateStore) error {
			iter, err := state.AllocsByNode(ws, args.NodeID)
			if err != nil {
	...
	opts := blockingOptions{
		...
		run: func(ws memdb.WatchSet, store *state.StateStore) error {
			...
			if node != nil {
				...
				iter, err := store.AllocsByNode(ws, args.NodeID)
				if err != nil {
					return structs.NewErrRPCCoded(400, err.Error())
				}
				allocs = iter.Slice()
...
func (n *Node) createNodeEvals(node *structs.Node, nodeIndex uint64) ([]string, uint64, error) {
	...
	iter, err := snap.AllocsByNode(nil, nodeID)
	if err != nil {
		return nil, 0, err // TODO(tgross): impossible node ID?
	}
	allocs := iter.Slice()
	sysJobsIter, err := snap.JobsByScheduler(nil, "system")
	if err != nil {
		return nil, 0, err // TODO(tgross): scheduler type?
	}
