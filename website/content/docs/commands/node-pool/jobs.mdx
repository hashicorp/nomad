...
```shell-session
$ nomad node pool -namespace default jobs prod
...
```shell-session
$ nomad node pool -namespace '*' jobs prod
...
```shell-session
$ nomad node pool jobs -per-page 2
ID       Type     Priority  Status          Submit Date
job1     service  80        running         07/25/17 15:47:11 UTC
job2     batch    40        complete        07/24/17 19:22:11 UTC
...
nomad node pool list -per-page 2 -page-token job3
