...
func (s *Sandbox) runCmd(env *parameters) error {
	...
	if !isPathWithin(env.AllocDir, env.TaskDir) {
...
func genWalkInspector(rootDir string) (fs.WalkDirFunc, error) {
	...
	walkFn = func(path string, entry fs.DirEntry, err error) error {
		...
		if !isPathWithin(rootDir, toCheck) {
...
func isPathWithin(rootPath, toCheckPath string) bool {
	...
	return os.SameFile(rootStat, checkStat)
}
