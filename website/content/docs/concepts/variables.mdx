...
of time by using a lock over it. Once a variable is locked, it can be read by
...
only one of them must set one instance as a leader.

Nomad offers, as part of the API, a helper that takes one variable and uses a lock
over it as a sync mechanism to run multiple instances but always keeping just one
running at any given time, using the following algorithm:
...
To see a live implementation, look for the [`nomad var loc`][] command.
...
[`nomad var loc`]: /nomad/docs/commands/var