...
// identityHook sets the alloc runner's Nomad workload identity token based on
// the signed identity stored on the Allocation

const (
	// wiTokenFile is the name of the file holding the Nomad token inside
	// NOMAD_SECRETS_DIR
...
func (h *identityHook) setDefaultToken() error {
	token := h.ar.alloc.SignedIdentities[h.ar.taskName]
	if token == "" {
		return nil
	}

	// Handle internal use and env var
	h.ar.setNomadToken(token)
...
func (h *identityHook) setAltToken(widspec *structs.WorkloadIdentity, rawJWT string) error {
	if widspec.Env {
		h.ar.envBuilder.SetWorkloadToken(widspec.Name, rawJWT)
	}

	if widspec.File {
		tokenPath := filepath.Join(h.tokenDir, fmt.Sprintf("nomad_%s.jwt", widspec.Name))
		if err := users.WriteFileFor(tokenPath, []byte(rawJWT), h.ar.Task().User); err != nil {
