...
Most Nomad workloads need access to config values or secrets. Nomad provides a `template`
block to run render such configuration on behalf of tasks, but has left the role of storing that
configuration to external services such as [HashiCorp Consul] and [HashiCorp Vault].

Nomad's Secure Variables feature provides the option to securely store configuration at
file-like paths directly in Nomad's state store. The contents of these secrets are
...
data. For example, do not store batch job results as Secure Variables - these should be stored in an external
...
encryption, continue using Vault.
...
Secure Variables found at Nomad-owned paths with the prefix `nomad/jobs/`,
followed by the job ID, task group name, and task name.

```
nomad/jobs/$job_id/$task_group/$task_name
nomad/jobs/$job_id/$task_group
nomad/jobs/$job_id
nomad/jobs
```
...
```
nomad/jobs/example/cache/redis
nomad/jobs/example/cache
nomad/jobs/example
nomad/jobs
```
