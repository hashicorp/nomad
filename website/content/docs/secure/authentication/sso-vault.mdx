...
In this tutorial, you will setup Vault as an OIDC provider and Nomad as its
client.

-> **Note:** Nomad operates as a confidential OIDC client in this tutorial. To
...
For the purpose of this tutorial, you will use the `root` token to work with Vault
...
1. Create a Vault OIDC provider named `my-provider` and provide it a list of client IDs and scopes. The provider grants access to the `nomad` client.
   ...
   <Tab heading="HCP Vault Dedicated" group="hcp">
      ...
      Vault Dedicated uses the private address for the `issuer` URL. For this tutorial, you will override the `issuer` value to match the public URL.
...
In this tutorial, you configured Vault as an OIDC provider with Nomad as a
