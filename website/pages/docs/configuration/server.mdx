...
- `enable_event_broker` `(bool: true)` - Specifies if this server's state store
...
- `event_buffer_size` `(int: 100)` - Specifies the amount of events generated
  by the server's state store to be held in memory. 
...
- `durable_event_count` `(int: 100)` - Specifies the amount of events to
  persist during snapshot generation. This provides a given servers event
  stream durability during restarts by persiting a backlog of events to be
  repopulated when the server comes back online.
