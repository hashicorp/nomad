	...
	testCases := []struct {
	...
	}{
		{
			...
			expected: &Job{
				...
				ConsulToken:       pointer.Of(""),
				ConsulNamespace:   pointer.Of(""),
				VaultToken:        pointer.Of(""),
				VaultNamespace:    pointer.Of(""),
				NomadTokenID:      pointer.Of(""),
				Status:            pointer.Of(""),
				StatusDescription: pointer.Of(""),
				Stop:              pointer.Of(false),
				Stable:            pointer.Of(false),
				Version:           pointer.Of(uint64(0)),
				CreateIndex:       pointer.Of(uint64(0)),
				ModifyIndex:       pointer.Of(uint64(0)),
				JobModifyIndex:    pointer.Of(uint64(0)),
				...
				TaskGroups: []*TaskGroup{
					{
						...
						RestartPolicy: &RestartPolicy{
							Delay:    pointer.Of(15 * time.Second),
						...
						Tasks: []*Task{
							{
								KillTimeout:   pointer.Of(5 * time.Second),
		...
		},
		{
			...
			input: &Job{
				ID:          pointer.Of("example_template"),
				Name:        pointer.Of("example_template"),
						...
						Tasks: []*Task{
							{
								...
								Templates: []*Template{
									{
										EmbeddedTmpl: pointer.Of("---"),
										DestPath:     pointer.Of("local/file.yml"),
									},
									{
										EmbeddedTmpl: pointer.Of("FOO=bar\n"),
										DestPath:     pointer.Of("local/file.env"),
						...
						Tasks: []*Task{
							{
								...
								Templates: []*Template{
									{
										SourcePath:   pointer.Of(""),
										DestPath:     pointer.Of("local/file.yml"),
										EmbeddedTmpl: pointer.Of("---"),
										ChangeMode:   pointer.Of("restart"),
										ChangeSignal: pointer.Of(""),
										Splay:        pointer.Of(5 * time.Second),
										Perms:        pointer.Of("0644"),
										Uid:          pointer.Of(-1),
										Gid:          pointer.Of(-1),
										LeftDelim:    pointer.Of("{{"),
										RightDelim:   pointer.Of("}}"),
										Envvars:      pointer.Of(false),
										VaultGrace:   pointer.Of(time.Duration(0)),
									},
									{
										SourcePath:   pointer.Of(""),
										DestPath:     pointer.Of("local/file.env"),
...

		{
			...
			expected: &Job{
				...
				Periodic: &PeriodicConfig{
					...
					Spec:            pointer.Of(""),
					SpecType:        pointer.Of(PeriodicSpecCron),
					ProhibitOverlap: pointer.Of(false),
					TimeZone:        pointer.Of("UTC"),
				},
			},
		},

		{
			...
			input: &Job{
				...
				Update: &UpdateStrategy{
					...
					HealthCheck:      pointer.Of("checks"),
					MinHealthyTime:   pointer.Of(10 * time.Second),
					HealthyDeadline:  pointer.Of(6 * time.Minute),
					ProgressDeadline: pointer.Of(7 * time.Minute),
					AutoRevert:       pointer.Of(false),
					Canary:           pointer.Of(0),
					AutoPromote:      pointer.Of(false),
				},
				TaskGroups: []*TaskGroup{
					{
						...
						Update: &UpdateStrategy{
							Stagger:        pointer.Of(2 * time.Second),
							MaxParallel:    pointer.Of(2),
							HealthCheck:    pointer.Of("manual"),
							MinHealthyTime: pointer.Of(1 * time.Second),
							AutoRevert:     pointer.Of(true),
							Canary:         pointer.Of(1),
							AutoPromote:    pointer.Of(true),
						},
						...
						Tasks: []*Task{
							{
								...
								RestartPolicy: &RestartPolicy{
									...
									Delay:    pointer.Of(1 * time.Second),
								},
							},
						},
					},
...
func TestJobs_Revert(t *testing.T) {
	...
	_, _, err = jobs.Revert(*job.ID, 0, pointer.Of(uint64(10)), nil, "", "")
	if err == nil || !strings.Contains(err.Error(), "enforcing version") {
		t.Fatalf("expected enforcement error: %v", err)
	}

	// Works at correct index
	revertResp, wm, err := jobs.Revert(*job.ID, 0, pointer.Of(uint64(1)), nil, "", "")
	if err != nil {
		t.Fatalf("err: %s", err)
	}
...
func TestJobs_ScaleInvalidAction(t *testing.T) {
	...
	// Register test job
	job := testJob()
	job.ID = pointer.Of("TestJobs_Scale")
	_, wm, err := jobs.Register(job, nil)
	require.NoError(err)
	assertWriteMeta(t, wm)

	// Perform a scaling action with bad group name, verify error
	_, _, err = jobs.Scale(*job.ID, "incorrect-group-name", pointer.Of(2),
...
func TestJobs_NewBatchJob(t *testing.T) {
	...
	expect := &Job{
		...
		Type:     pointer.Of(JobTypeBatch),
		Priority: pointer.Of(5),
	}
...
func TestJobs_NewSysbatchJob(t *testing.T) {
	...
	expect := &Job{
		...
		ID:       pointer.Of("job1"),
		Name:     pointer.Of("myjob"),
		Type:     pointer.Of(JobTypeSysbatch),
		Priority: pointer.Of(5),
	}
...
func TestJobs_ScaleAction(t *testing.T) {
	...
	_, _, err := jobs.Scale(id, "missing", pointer.Of(newCount), "this won't work",
		false, nil, nil)
	require.Error(err)
	...
	scalingResp, wm, err := jobs.Scale(id, groupName,
		pointer.Of(newCount), "need more instances", false,
