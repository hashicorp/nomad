...
- `enabled_schedulers` `(array<string>: ["service", "batch", "system",
  "sysbatch"])` - Specifies which sub-schedulers this server will handle. This
  can be used to restrict the evaluations that worker threads will dequeue for
  processing.
