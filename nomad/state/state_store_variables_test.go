...
func TestStateStore_GetVariables(t *testing.T) {
	...
	ws := memdb.NewWatchSet()
	iter := testState.GetVariablesByNamespace(ws, svs[0].Namespace)
	...
	iter = testState.GetVariablesByNamespace(ws, svs[1].Namespace)
	...
	// variables.
	iter = testState.GetVariablesByNamespace(ws, "pony-club")
	var count3 int
	for range iter.All() {
		count3++
	}
...
func TestStateStore_ListVariablesByNamespaceAndPrefix(t *testing.T) {
	...
	t.Run("ByNamespace", func(t *testing.T) {
		...
		for _, tC := range testCases {
			t.Run(tC.desc, func(t *testing.T) {
				iter := testState.GetVariablesByNamespace(ws, tC.namespace)
				var count int = 0
				for sv := range iter.All() {
					count++
	...
	t.Run("ByNamespaceAndPrefix", func(t *testing.T) {
		...
		for _, tC := range testCases {
			t.Run(tC.desc, func(t *testing.T) {
				iter := testState.GetVariablesByNamespaceAndPrefix(ws, tC.namespace, tC.prefix)
				var count int = 0
				for sv := range iter.All() {
					count++
					must.Eq(t, tC.namespace, sv.Namespace)
	...
	t.Run("ByPrefix", func(t *testing.T) {
		...
		for _, tC := range testCases {
			t.Run(tC.desc, func(t *testing.T) {
				iter := testState.GetVariablesByPrefix(ws, tC.prefix)
				var count int = 0
				for sv := range iter.All() {
					count++
					must.True(t, strings.HasPrefix(sv.Path, tC.prefix))
...
func TestStateStore_ListVariablesByKeyID(t *testing.T) {
	...
	ws := memdb.NewWatchSet()
	iter := testState.GetVariablesByKeyID(ws, keyID)
