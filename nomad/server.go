...
type Server struct {
	...
	*rpcHandler
	...
	peers      map[string][]*peer.Parts
	localPeers map[raft.ServerAddress]*peer.Parts
	...
	peerPartsCache *peer.PartsCache
...
func NewServer(config *Config, consulCatalog consul.CatalogAPI, consulConfigFunc consul.ConfigAPIFunc) (*Server, error) {
	...
	s := &Server{
		...
		peers:                   make(map[string][]*peer.Parts),
		localPeers:              make(map[raft.ServerAddress]*peer.Parts),
		peerPartsCache:          peer.NewPartsCache(),
		bootstrapped:            &atomic.Bool{},
