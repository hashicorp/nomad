...
func ParseAttribute(input string) *Attribute {
	ll := len(input)
	if ll == 0 {
		return &Attribute{String: helper.PointerOf(input)}
	}
	...
	if err == nil {
		return &Attribute{Int: helper.PointerOf(i), Unit: unit}
	}

	// Try to parse as a float
	f, err := strconv.ParseFloat(numeric, 64)
	if err == nil {
		return &Attribute{Float: helper.PointerOf(f), Unit: unit}
	}

	// Try to parse as a bool
	b, err := strconv.ParseBool(input)
	if err == nil {
		return &Attribute{Bool: helper.PointerOf(b)}
	}
...
func (a *Attribute) Copy() *Attribute {
	...
	if a.Float != nil {
		ca.Float = helper.PointerOf(*a.Float)
	}
	if a.Int != nil {
		ca.Int = helper.PointerOf(*a.Int)
	}
	if a.Bool != nil {
		ca.Bool = helper.PointerOf(*a.Bool)
	}
	if a.String != nil {
		ca.String = helper.PointerOf(*a.String)
	}
