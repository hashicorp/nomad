...
Preemption is enabled by default in Nomad 0.9. Operators can use the [scheduler config]/1/api-docs/operator#update-scheduler-configuration) API endpoint to disable preemption.
...
Operators can use the [allocation API]/1/api-docs/allocations#read-allocation) or the `alloc status` command to get visibility into
