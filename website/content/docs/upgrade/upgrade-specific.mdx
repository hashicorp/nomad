...
Nomad will no longer create an implicit Consul block for workloads that don't
register services with Consul. Jobs that require Consul tokens for template
rendering will be required to provide an identity.

Tasks that include template blocks utilizing Consul API functionality such as
`key`, `node`, and `service` will now need to specify an [`identity`][] block,
so the template runner is presented with workload permissions to access the
Consul API.
