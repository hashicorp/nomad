...
Nomad will find a client with enough spare capacity to reserve 2 cores
exclusively for the task. Unlike the `cpu` field, the task will not share CPU
time with any other tasks managed by Nomad on the client.
