...
system gc` command, Nomad ignores the specified time threshold.
...
allocations are created by specific evaluations, allocations are garbage
collected when their evaluations are garbage collected. This also means that
evaluations cannot be garbage collected until their allocations are terminal.
...
- Terminal state
  ...
  thresholds have no been reached.
...
- Server garbage collection
  ...
  removes all terminal objects on all servers and clients, ignoring thresholds.
