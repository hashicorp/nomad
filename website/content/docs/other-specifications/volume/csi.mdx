...
Next, use the [`volume create`][] command and the `volume.hcl` file to create
the volume.
...
Review the volume's current capacity with the [`volume status`][] command.
...
Next, to increase volume capacity, update the `capacity_min` and `capcity_max`
fields in the `volume.hcl` file.
...
Run the `volume create` command to expand the volume.
...
Use the `volume status` command to make sure the volume has the new capacity.
