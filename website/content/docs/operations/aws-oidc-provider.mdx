---
layout: docs
page_title: Authenticate Nomad jobs with AWS IAM identity as an OIDC provider
description: |-
  Integrate Nomad as an OpenID Connect (OIDC) provider with AWS IAM Identity.
---

# Authenticate Nomad jobs with AWS IAM identity as an OIDC provider

This page describes how to integrate Nomad with AWS IAM as an OpenID Connect (OIDC) provider.
You can configure Nomad jobs to present a JSON Web Token (JWT) that the Nomad server authenticates 
and passes to AWS IAM so that the job can access AWS services.
...
The process to integrate Nomad with AWS IAM identity consists of the following steps:
...
The following example sets S3 bucket access.
