...
const (
	// duplicateAllocIndexErrorString is the error message string used when the
	// plan applier finds a duplicate alloc index within a job version.
	duplicateAllocIndexErrorString = "duplicate alloc index found in plan"
)
...
func evaluatePlan(pool *EvaluatePool, snap *state.StateSnapshot, plan *structs.Plan, logger log.Logger) (*structs.PlanResult, error) {
	...
	// Check to ensure this job plan does not create duplicate allocation
	// indexes within a job version.
...
func evaluatePlanAllocIndexes(stateSnap *state.StateSnapshot, plan *structs.Plan) error {
	...
	// Iterate the allocations currently stored within state. Any found that
	// belong to the planned job version and that are not terminal or unknown
	// are considered liable for duplicate alloc index checking.
	...
	// This exists solely to handle plans generated by clients that disconnect
	// which are running allocations that have max_client_disconnect
	// configured.
	nodeAllocSet := set.New[string](0)

	// Iterate all the node allocations within the plan. Plans generated due to
	// disconnected clients and allocations in the unknown state will include
	// both the unknown and replacement allocation within the plan
	// NodeAllocation.
	//
	// We cannot rely on a specific ordered iteration of the node map, nor the
	// allocation slice. Therefore, we must use nodeAllocSet along with
	// allocIndexMap to find duplicate alloc indexes.
	for _, nodeAllocs := range plan.NodeAllocation {
		for _, nodeAlloc := range nodeAllocs {
			// If the allocation is in an unknown state, we need to remove it
			// from the allocation tracking map. It will exist within state as
			// running and matching the plan version.
			//
			// All other states, we use the intermediate set and look for
			// initial duplications within the node allocation section of the
			// plan.
	...
	// Perform the final duplication check which compares the allocations being
	// added to the running allocations from state whose job version matches
	// the job version within the plan.
	for _, allocName := range nodeAllocSet.Slice() {
		if _, ok := allocIndexMap[allocName]; ok {
			return errors.New(duplicateAllocIndexErrorString)
		}
	}
