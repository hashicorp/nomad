...
- `error_on_missing_key` `(bool: false)` - Specifies how the template behaves
  when attempting to index a struct or map key that does not exist. If set to true, the task will fail if a template tries to access a key that doesn't exist in a struct or map. If set to false, an empty value will be used in the template instead.
