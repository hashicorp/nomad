...
- `entity_alias` `(string: "")` - Specifies the entity alias to use when creating
  Vault tokens. This allows multiple tokens to be created as a single Vault client.
  This value can be overriden by the `entity_alias` value in [vault job configuration](/docs/job-specification/vault#entity_alias).
