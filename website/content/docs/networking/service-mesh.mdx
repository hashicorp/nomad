...
external services are called upstreams and are declared using the [`upstreams`]
block.
...
3. the proxy for the `api` service forwads the request to the local port in the
   allocation.
...
participate there are specialized proxies called gateways that can be used for
mesh-wide connectivity. The types of gateways provided by Consul Service Mesh
are:

- **mesh gateways** allow communication between different service meshes.
...
- **egress gateways** allow services inside the mesh to communication with
  services outside the mesh.

Nomad can deploy these gateways using the [`gateway`] block. Nomad injects an
Envoy proxy task to any `group` with a `gateway` service.
