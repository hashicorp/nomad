...
- `denied_host_uids` - (Optional) Range of host uids to deny. If a "user"
  value is specified in task configuration and that user has a user id in
  the given ranges, the task will error before starting.
...
- `denied_host_gids` - (Optional) Range of host gids to deny. If a "user"
  value is specified in task configuration and that user has is part of
...
- `no_chroot` `(bool: optional)` - Defaults to `false`. When `true`, the driver does not
  use chroot for file system isolation. This is an experimental option.
