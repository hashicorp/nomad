...
~> On Linux the `task.user` field cannot be set on a Task using the `raw_exec` driver if
the Nomad client has been hardened according to the [production][hardening] guide.
On Windows, when Nomad is running as [system service][service] less privileged service
user can be specified (example: `NT AUTHORITY\LocalService`, `NT AUTHORITY\NetworkService`)
