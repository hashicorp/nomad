...
- `common_plugins_dir` `(string: "")` - Specifies the directory wherein
  plugins conforming to the common plugins interface (TODO) should be placed.
  When this parameter is empty, Nomad generates the path using the
  [top-level `data-dir`][top_level_data_dir] suffixed with `common_plugins`,
  like `"/opt/nomad/common_plugins"`. This must be an absolute path.
