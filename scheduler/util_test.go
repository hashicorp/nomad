...
func TestShuffleNodes(t *testing.T) {
	...
	must.False(t, reflect.DeepEqual(nodes, orig))

	nodes2 := make([]*structs.Node, len(nodes))
	copy(nodes2, orig)
	shuffleNodes(plan, 1000, nodes2)

	must.True(t, reflect.DeepEqual(nodes, nodes2))
}
...
func TestSetStatus(t *testing.T) {
	...
	newEval = h.Evals[0]
	must.True(t, reflect.DeepEqual(newEval.FailedTGAllocs, metrics),
	...
	newEval = h.Evals[0]
	must.True(t, reflect.DeepEqual(newEval.QueuedAllocations, queuedAllocs),
		must.Sprintf("setStatus() didn't set failed task group metrics correctly: %v", newEval))
...
func TestInplaceUpdate_ChangedTaskGroup(t *testing.T) {
	...
	*tg = *job.TaskGroups[0]
	...
	must.True(t, len(unplaced) == 1 && len(inplace) == 0, must.Sprintf("inplaceUpdate incorrectly did an inplace update"))
	must.MapEmpty(t, ctx.plan.NodeAllocation, must.Sprintf("inplaceUpdate incorrectly did an inplace update"))
}
...
func TestInplaceUpdate_AllocatedResources(t *testing.T) {
	...
	must.True(t, len(unplaced) == 0 && len(inplace) == 1, must.Sprintf("inplaceUpdate incorrectly did not perform an inplace update"))
	must.MapNotEmpty(t, ctx.plan.NodeAllocation, must.Sprintf("inplaceUpdate incorrectly did an inplace update"))
...
func TestInplaceUpdate_Success(t *testing.T) {
	...
	*tg = *job.TaskGroups[0]
	...
	must.True(t, len(unplaced) == 0 && len(inplace) == 1, must.Sprintf("inplaceUpdate did not do an inplace update"))
	must.Eq(t, 1, len(ctx.plan.NodeAllocation), must.Sprintf("inplaceUpdate did not do an inplace update"))
	...
	must.Eq(t, 3, len(a.Job.TaskGroups[0].Tasks[0].Services),
		must.Sprintf("Expected number of services: %v, Actual: %v", 3, len(a.Job.TaskGroups[0].Tasks[0].Services)))
...
func TestTaskGroupConstraints(t *testing.T) {
	...
	must.True(t, reflect.DeepEqual(actConstrains.constraints, expConstr),
		must.Sprintf("taskGroupConstraints(%v) returned %v; want %v", tg, actConstrains.constraints, expConstr))
	must.True(t, reflect.DeepEqual(actConstrains.drivers, expDrivers),
		must.Sprintf("taskGroupConstraints(%v) returned %v; want %v", tg, actConstrains.drivers, expDrivers))
}
...
func TestProgressMade(t *testing.T) {
	...
	must.False(t, progressMade(nil) || progressMade(noopPlan), must.Sprintf("no progress plan marked as making progress"))
...
func TestDesiredUpdates(t *testing.T) {
	...
	must.True(t, reflect.DeepEqual(desired, expected), must.Sprintf("desiredUpdates() returned %#v; want %#v", desired, expected))
}
...
func TestUtil_UpdateNonTerminalAllocsToLost(t *testing.T) {
	...
	must.True(t, reflect.DeepEqual(allocsLost, expected), must.Sprintf("actual: %v, expected: %v", allocsLost, expected))
