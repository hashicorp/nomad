...
func (f *CPUFingerprint) Fingerprint(cfg *config.Config, node *structs.Node) (bool, error) {
	...
	node.Attributes["cpu.frequency"] = fmt.Sprintf("%.1f", mhz)
	f.logger.Printf("[DEBUG] fingerprint.cpu: frequency: %.1f MHz", mhz)
	...
	node.Attributes["cpu.totalcompute"] = fmt.Sprintf("%.1f", tt)
