...
operation is desired. An optional `description` can be provided when using
`-disable` flag.
...
- `-description`: Description when marking a node as ineligible for new allocations.
...
Disable scheduling eligibility with description on the local node:

```shell-session
$ nomad node eligibility -disable -description "Marking node as ineligible due to multiple plan rejections" -self
