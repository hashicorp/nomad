...
func initialize() {
	...
	if err != nil {
		panic("nomadapi.freeport: ephemeral port range detection failed: " + err.Error())
	}
	if effectiveMaxBlocks < 0 {
		panic("nomadapi.freeport: no blocks of ports available outside of ephemeral range")
	}
	if lowPort+effectiveMaxBlocks*blockSize > 65535 {
		panic("nomadapi.freeport: block size too big or too many blocks requested")
	}
...
func alloc() (int, net.Listener) {
	...
	panic("nomadapi.freeport: cannot allocate port block")
}
...
func Take(n int) (ports []int, err error) {
	if n <= 0 {
		return nil, fmt.Errorf("nomadapi.freeport: cannot take %d ports", n)
	}
	...
	if n > total {
		return nil, fmt.Errorf("nomadapi.freeport: block size too small")
	}

	for len(ports) < n {
		for freePorts.Len() == 0 {
			if total == 0 {
				return nil, fmt.Errorf("nomadapi.freeport: impossible to satisfy request; there are no actual free ports in the block anymore")
			}
			condNotEmpty.Wait()
		}
...
func logf(severity string, format string, a ...interface{}) {
	_, _ = fmt.Fprintf(os.Stderr, "["+severity+"] nomadapi.freeport: "+format+"\n", a...)
}
