...
- `max_parallel` `(int: 1)` - Specifies the number of allocations within a task
  group that can be updated at the same time. The task groups themselves are
  updated in parallel.

  - `max_parallel = 0` - Specifies that the allocation should use forced updates
    instead of deployments

  ~> **Note:** System jobs with `canary` set perform destructive updates, and
  thus will only place up to `max_parallel` allocations *or* enough allocations
  to place on `canary` percent of _feasible_ nodes, whichever is lower.
  Operators wanting to use `canary` feature with system jobs should adjust the
  `max_parallel` value accordingly if they want to deploy all the desired
  canaries.
...
- `canary` `(int: 0)` - Specifies that changes to the job that would result in
  ...
  nodes to which allocations will be deployed. System jobs do not support more
  than one allocation per node, so effectively setting `canary` to a positive
  integer means this percentage of eligible nodes will get a new version of the
  job. Setting `canary` to 100 updates the job on all nodes. Percentage of nodes
  is always rounded up to the nearest integer.
