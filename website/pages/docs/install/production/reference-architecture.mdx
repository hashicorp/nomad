...
Additional documentation is available to learn more about [Nomad server federation](https://learn.hashicorp.com/tutorials/nomad/federation).
...
Additional documentation is available to learn more about [cluster sizing and failure tolerances](/docs/internals/consensus#deployment-table) as well as [outage recovery](https://learn.hashicorp.com/tutorials/nomad/outage-recovery).
...
In the event of a region-level failure (which would contain an entire Nomad server cluster), clients will still be able to submit jobs to another region that is properly federated. However, there will likely be data loss as Nomad server clusters do not replicate their data to other region clusters. See [Multi-region Federation](https://learn.hashicorp.com/tutorials/nomad/federation) for more setup information.
...
[acl]: https://learn.hashicorp.com/collections/nomad/access-control
[sentinel]: https://learn.hashicorp.com/tutorials/nomad/sentinel
