...
Nomad now enables [PKCE](https://oauth.net/2/pkce/) (Proof Key for Code
Exchange) by default for new or updated OIDC auth methods. Existing auth
methods remain unaffected until changed using the
...
The [`OIDCDisablePKCE`](/nomad/api-docs/acl/auth-methods#oidcdisablepkce)
